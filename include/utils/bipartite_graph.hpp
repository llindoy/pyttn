#ifndef UTILS_BIPARTITE_GRAPH_HPP_
#define UTILS_BIPARTITE_GRAPH_HPP_
...
template <typename T, typename V> std::ostream& operator<<(std::ostream& os, const bipartite_graph<T, V>& op);
...
public:
    static void generate_connected_subgraphs(const bipartite_graph<T, Val>& G, std::list<bipartite_graph<T, Val>>& SG)
    {
        ...
        for(size_t i = 0; i < G.N(); ++i){if(G.U_edges(i).size() == 0){++nodes_filled; Uc[i] = true;}}
        for(size_t i = 0; i < G.M(); ++i){if(G.V_edges(i).size() == 0){++nodes_filled; Vc[i] = true;}}
...
#endif //UTILS_BIPARTITE_GRAPH_HPP_//
