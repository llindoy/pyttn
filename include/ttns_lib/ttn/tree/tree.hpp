#ifndef TTNS_DATASTRUCTURES_TREE_HPP
#define TTNS_DATASTRUCTURES_TREE_HPP
...
public:
    ...
    node_reference operator()(const std::list<size_type>& tree_index)
    {
        ...
        if(tree_index.size() == 0){return m_nodes[0];}
        else{return m_nodes[0].at(tree_index.begin(), tree_index.end());}
    }
    ...
    {
        ASSERT(m_nodes.size() != 0, "Failed to access root node of tree. The tree is empty.");
        if(tree_index.size() == 0){return m_nodes[0].id();}
        else{return m_nodes[0].id_at(tree_index.begin(), tree_index.end());}
    }
    ...
    {
        ASSERT(m_nodes.size() != 0, "Failed to access root node of tree. The tree is empty.");
        if(first == last){return m_nodes[0];}
        else{return m_nodes[0].at(++first, last);}
    }
    template <typename IT> 
...
#endif  //TTNS_DATASTRUCTURES_TREE_HPP//
