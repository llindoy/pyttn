#ifndef TTNS_SOP_CENTRAL_SPIN_HAMILTONIAN_HPP
#define TTNS_SOP_CENTRAL_SPIN_HAMILTONIAN_HPP
...
public:
    ...
    central_spin_base(size_t spin_index, real_type eps, real_type delta, size_t N) : m_eps(eps), m_delta(delta), m_spin_index(spin_index), m_mode_dims(N) {}
...
//a class for handling the generation of the central_spin hamiltonian with homogeneous bath coupling
template <typename value_type> 
...
public:
    ...
    central_spin_homogeneous(size_t spin_index, real_type eps, real_type delta, const std::vector<real_type>& _A) : base_type(spin_index, eps, delta, _w.size()), m_w(_w), m_g(_g)
    {
        ASSERT(_w.size()  == _g.size(), "Invalid boson parameters.")
    }
