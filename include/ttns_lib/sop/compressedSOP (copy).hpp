...
public:
    ...
    sPOP as_product_operator(const std::vector<std::pair<size_t, size_t>>& copdict, const std::vector<std::vector<std::string>>& opdict) const
    {
        ...
        for(auto ind : m_op_indices)
        {
            ...
            std::cerr << ind << " " << copdict.size() << " " << sOP(opdict[std::get<0>(rv)][std::get<1>(rv)], std::get<0>(rv)) << std::endl;
        }
        return ret;
    }
...
//a generic sum of product operator object.  Currently there is a bug when we have two operators acting on different modes of a composite mode. 
template <typename T>
class compressedSOP
{
public:
...
public:
    ...
    compressedSOP(const SOP<T>& op, const system_modes& sysinf) 
    {
        try
        {
            initialise(op, sysinf);
        }
        catch(const std::exception& ex)
        {
            std::cerr << ex.what() << std::endl;
...
protected:
    size_t find_term(const composite_dict_type& dict, const std::pair<size_t, size_t>& elem)
    {
        auto it = std::find(dict.begin(), dict.end(), elem);
        ASSERT(it != dict.end(), "Failed to find term in dictionary.");
...
public:
    void initialise(const SOP<T>& o, const system_modes& sysinf)
    {
        try
        {
            ASSERT(o.nmodes() <= sysinf.nprimitive_modes(), "Cannot initialise compressed SOP object from the specified SOP and system_modes objects.  Incompatible numbers of modes");
            m_label = o.label();
            resize(o.nterms(), sysinf.nmodes());

            //set up the operator dictionary objects
            //First resize the composite operator dictionary.  This is a vector of vectors of pairs pointing to the correct m_op_dict element
            m_composite_op_dict.resize(sysinf.nmodes());

            //Stores the primitive operator dictionary.  
            ...
            for(const auto& pop : o)
            {
                ...
                {
                    size_t i = std::get<1>(op);
                    auto cminf = sysinf.primitive_mode_index(i);
                    size_t mode = std::get<0>(cminf);
                    size_t cmode = sysinf.mode_index(mode);

                    size_t cindex = find_term(m_composite_op_dict[cmode], std::make_pair(i, std::get<0>(op)));
                    terms[cmode].push_back(cindex);
                }
                
                //iterate over each term and store the mode and the compositeSiteOperator.  
                if(pop.first.contains_jordan_wigner_string())
                {
                    for(size_t i = 0; i < o.nmodes(); ++i)
                    {
                        //get the information about the primitive mode i
                        auto cminf = sysinf.primitive_mode_index(i);
                        size_t mode = std::get<0>(cminf);

                        //and find out the tree mapped mode
                        size_t cmode = sysinf.mode_index(mode);

                        //now add on the jordan wigner indices 
                        if(pop.first.prepend_jordan_wigner_string(i))
                        {
                            size_t cindex = find_term(m_composite_op_dict[cmode], std::make_pair(i, o.jordan_wigner_index(i)));
                            terms[cmode].push_back(cindex);
                        }
                    }
                }
    ...
    void insert_identities(const system_modes& sysinf)
    {
        try
        {
            ...
            for(size_t mode = 0; mode < m_mode_operators.size(); ++mode)
            {
                ...
                //if we didn't find an identity term we will need to add it both the operator dictionary and the composite mode dictionary.
                if(!identity_found)
                {
    ...
    site_ops_type site_operators() const
    {
        ...
        for(size_t i = 0; i < m_mode_operators.size(); ++i)
        {
            for(const auto& op : m_mode_operators[i])
            {
                ...
                std::cerr << op.first.as_product_operator(m_composite_op_dict[i], m_op_dict) << std::endl;
                std::cerr << op.second << std::endl;
            }
        }
        return ret;
    }
    ...
    SOP<T> sop() const
    {
        ...
        {   
            for(const auto& op : m_mode_operators[i])
            {
                ...
                if(top.size() == 1 && top.ops().front().op() == std::string("id")){}
                else
                {
                    op.second.get(rinds);
