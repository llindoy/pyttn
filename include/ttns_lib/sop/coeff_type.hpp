...
public:
    ...
    {
        m_funcs.push_back(std::make_pair(T(1.0), function_type([f](real_type t){return T(f(t));})));
    }
    ...
    coeff(const coeff<U>& o)
    {
        ...
        for(size_t i = 0; i < o.funcs().size(); ++i)
        {
            ...
            m_funcs.push_back(std::make_pair(T(std::get<0>(ai)), function_type([aif](real_type t){return T(aif(t));})));
        }
    }
        ...
        else
        {
            ...
            for(size_t i = 0; i < m_funcs.size(); ++i)
            {
                ...
                for(size_t j = 0; j< o.m_funcs.size(); ++j)
                {
                    ...
                    funcs.push_back(std::make_pair(std::get<0>(ai)*std::get<0>(bj), function_type([aif, bjf](real_type t){return aif(t)*bjf(t);})));
                }
            }
            m_funcs.clear();
            m_funcs = funcs;
...
ttns::literal::coeff<V> operator*(const ttns::literal::coeff<T>& a, const ttns::literal::coeff<U>& b)
{
    ...
    if(linalg::abs(b.constant()) > 1e-14)
    {
        for(size_t i = 0; i < a.funcs().size(); ++i)
        {
            ...
            const auto& aif = std::get<1>(ai);
            ret.funcs().push_back(std::make_pair(std::get<0>(ai)*b.constant(), function_type([aif](real_type t){return aif(t);})));
        }
    }
    for(size_t i = 0; i < a.funcs().size(); ++i)
    {
        ...
        for(size_t j = 0; j< b.funcs().size(); ++j)
        {
            ...
            const auto& bjf = std::get<1>(bj);
            ret.funcs().push_back(std::make_pair(std::get<0>(ai)*std::get<0>(bj), function_type([aif, bjf](real_type t){return aif(t)*bjf(t);})));
        }
    }
    return ret;
}
