#ifndef TTNS_LIB_SOP_AUTOSOP_NODE_HPP
#define TTNS_LIB_SOP_AUTOSOP_NODE_HPP
...
template <typename T> class autoSOP;
...
template <typename T> class node_op_info;
...
public:
    ...
    void clear()
    {
        for(auto& sp : m_spf){sp.clear();}  
        m_spf.clear();
        m_spf.shrink_to_fit();
        for(auto& m : m_mf){m.clear();}  
        m_mf.clear();
        m_mf.shrink_to_fit();
    }
...
public:
    ...
    public:
        ...
        {
            ref_vec ret;
            for(auto v : m_state){ret.push_back(std::reference_wrapper<dtype>(*v));}
            return ret;
        }
...
#endif  //TTNS_LIB_SOP_AUTOSOP_NODE_HPP
