...
def siam_dynamics(Nb, chi, dt, chiU = None, beta = None, Ncut = 20, nstep = 1, Nw = 7.5, geom='star', ofname='sbm.h5', degree = 1, adaptive=True, spawning_threshold=1e-5, unoccupied_threshold=1e-4, nunoccupied=0, init_state = 'up'):
    if chiU is None:
        chiU = chi
    ...
    pars = h5py.File("TOV_on_gold_AIM_20K.hdf5")
    T = np.array(pars.get('h_'+str(Nb)+'_bath_orbitals'))
    U = np.array(pars.get('U'))
    pars.close()
    ...
    gf = np.array(gf)
    ge = np.array([x for x in reversed(ge)])
    wf = np.array(wf)
    we = np.array([x for x in reversed(we)])
    ...
    for i in range(Nb):
        ...
        op += fOP("n", N-2-i)
        ops.append(sop_operator(op, A, sysinf))
        labels.append("n_d"+str(i))

        op = SOP(2*N)
        op += fOP("n", N+1+i)
        ops.append(sop_operator(op, A, sysinf))
        labels.append("n_u"+str(i))

    sweep = None
    if not adaptive:
        sweep = tdvp(A, h, krylov_dim = 12)
    else:
        sweep = tdvp(A, h, krylov_dim = 12, subspace_krylov_dim=12, subspace_neigs=6, expansion='subspace')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tmax', type=float, default=500)

    #output file name
    parser.add_argument('--fname', type=str, default='tov.h5')
