...
def pyrazine_dynamics(maxchi, tmax, dt, adaptive=True, spawning_threshold=1e-6, unoccupied_threshold=1e-4, nunoccupied=0, ofname="pyrazine.h5", output_skip=1):
    ...
    # flat mode ordering used for the trial MPS run: electronic mode + 24 (truncated) vibrational modes
    ...
    # rebuild sysinf with the full (untruncated) local dimensions, and reorder modes per the discovered tree
    sysinf[0] = generic_mode(2)
    for i in range(len(m)):
    ...
    from pyttn import set_topology_properties
    ...
    for i in range(1, nsteps):
        ...
        if i % output_skip == 0:
            output_results(ofname, timepoints, res, maxchi_arr, (t2 - t1), cmat)
...
if __name__ == "__main__":
    ...
    fname = args.fname if args.fname is not None else "results/pyrazine_" + str(args.chimax) + ".h5"
