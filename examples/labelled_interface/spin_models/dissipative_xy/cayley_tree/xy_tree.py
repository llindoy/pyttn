...
def cayley_edges(Nl, d=3):
    """Bethe-lattice/Cayley-tree connectivity: the root has ``d`` children, every
    subsequent node has ``d - 1`` children, built out to ``Nl`` layers. Returns the
    list of (parent, child) index pairs and the total number of sites."""
    edges, frontier, Ns = [], [0], 1
    for layer in range(Nl):
        new_frontier = []
        for parent in frontier:
            for _ in range(d if layer == 0 else d - 1):
                edges.append((parent, Ns))
...
def xytree_dynamics(Nl, alpha, wc, eta, chi, dt, method="unitary", Nb=50, Nw=4.0, geom="star", nbose=20, nbose_min=5, L=25, K=4, Lmin=4, Ecut=10.0, beta=None, nstep=1, ofname=None, degree=None, adaptive=True, spawning_threshold=2e-4, unoccupied_threshold=1e-4, nunoccupied=0):
    """Dynamics of a dissipative XY spin model on a Cayley tree, each spin coupled to its own independent bosonic bath, for any of the four supported methods. As in :mod:`xy_chain`, the only method-dependent piece is which bath decomposition to register per site; one :class:`~pyttn.oqs.OQSModel` with a bath per site and one :class:`~pyttn.oqs.MethodBuilder` call proposes the entire joint system+bath tree topology - replacing the hand-built Cayley topology and mode reordering machinery of the old ``cayley_helper.py`` - and (for heom/pseudomode) the Tr[rho] trace state, regardless of method."""
    ...
    state[result.topology.leaf_order().index(labels[0])] = 1 if unitary_like else 3
    A.set_state(state)
    ...
    ofname = ofname if ofname is not None else f"xytree_{method}.h5"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Dynamics of a dissipative XY spin model on a Cayley tree, for any of the unitary/tedopa/heom/pseudomode methods")
