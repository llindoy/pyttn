...
def chi_scaling_mps_binary_ternary(adaptive = False):
    ...
    chivs = []
    for chi in chis:
        m, std = sbm_dynamics_timing(nb, 1.0, 5, 1.0, 0.0, 1.0, chi, nbose, 0.01, nstep=nstep, degree = 1, compress = True, adaptive=adaptive)
    ...
    for chi in chis:
        m, std = sbm_dynamics_timing(nb, 1.0, 5, 1.0, 0.0, 1.0, chi, nbose, 0.01, nstep=nstep, degree = 2, compress = True, adaptive=adaptive)
        chivs.append(chi)
    ...
    chivs = []
    for chi in chis:
        m, std = sbm_dynamics_timing(nb, 1.0, 5, 1.0, 0.0, 1.0, chi, nbose, 0.01, nstep=nstep, degree = 3, compress = True, adaptive=adaptive)
        chivs.append(chi)
        timings_3.append(m)
