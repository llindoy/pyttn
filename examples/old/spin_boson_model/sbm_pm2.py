...
def sbm_dynamics(alpha, wc, s, eps, delta, chi, nbose, dt, nbose_min = None, beta = None, Ncut = 20, nstep = 1, Nw = 7.5, geom='star', ofname='sbm.h5', degree = 2, adaptive=True, spawning_threshold=2e-4, unoccupied_threshold=1e-4, nunoccupied=0, aaa_tol=1e-4):
    ...
    for i in range(Nb):
        ...
        H += Ek[i]*sOP("n1", i1)-Ek[i]*sOP("n2", i2)
        H += 2.0j*gk[i]*sOP("a1", i1)*sOP("a2", i2) - 1.0j*gk[i]*sOP("n1", i1) - 1.0j*gk[i]* sOP("n2", i2)
        H += Vk[i]*sOP("sl", 0) * sOP("adag1", i1)+Vk[i]*sOP("sl", 0) *sOP("a1", i1)  - Vk[i]*sOP("sr", 0) * sOP("adag2", i2)- Vk[i]*sOP("sr", 0) *sOP("a2", i2)
        H += 2.0j*Mk[i]*sOP("sr", 0)*sOP("a1", i1) - 1.0j*Mk[i]* sOP("sl", 0)*sOP("a1", i1) -1.0j*Mk[i]* sOP("sr", 0)*sOP("adag2", i2)
        H += 2.0j*Mk[i]*sOP("sl", 0)*sOP("a2", i2) - 1.0j*Mk[i] * sOP("sl", 0)*sOP("adag1", i1) - 1.0j*Mk[i] * sOP("sr", 0)*sOP("a2", i2)

    #set up the operator dictionary information
    opdict = operator_dictionary_complex(Nb+1)
    ...
    for i in range(Nb):
        opdict.insert(i+1, "n1", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))
        opdict.insert(i+1, "n2", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))
        opdict.insert(i+1, "a1a2", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))
        opdict.insert(i+1, "a1", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))
        opdict.insert(i+1, "a2", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))
        opdict.insert(i+1, "adag1", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))
        opdict.insert(i+1, "adag2", site_operator(np.zeros((b_mode_dims[i], b_mode_dims[i])), optype="matrix", mode=i+1))

    #setup the system information object
    sysinf = system_modes(Nb+1)
    sysinf[0] = generic_mode(mode_dims[0])
    for i in range(Nb):
        sysinf[i+1] = boson_mode(b_mode_dims[i])#boson_mode(mode_dims[2*i+1]), boson_mode(mode_dims[2*i+2])]
