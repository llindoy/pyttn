...
def setup_star_hamiltonian(eps, delta, g, w, Nb):
    ...
    H += 2*delta*sOP("sx", 0)
    for i in range(Nb):
        H += 2*np.sqrt(2)*g[i] * sOP("sz", 0)  * sOP("q", i+1)
        H += w[i] * sOP("n", i+1)
...
def setup_chain_hamiltonian(eps, delta, g, w, Nb):
    ...
    for i in range(Nb):
        if i == 0:
            H += 2*np.sqrt(2)*t[i]*sOP("sz", 0) * sOP("q", i+1)
        else:
...
def setup_ipchain_hamiltonian(eps, delta, g, w, Nb):
    ...
    class func_class:
        ...
        def __call__(self, ti):
            val = 2*self.t0*np.conj(self.U[:, 0])@(np.exp(-1.0j*ti*self.e)*self.U[:, self.i])
...
def sbm_dynamics(Nb, alpha, wc, s, eps, delta, chi, nbose, dt, Ncut = 20, nstep = 1, Nw = 10, geom='star', ofname='sbm.h5', beta=None):
    ...
    if geom  == 'chain':
        H, l = setup_chain_hamiltonian(eps, delta*renorm, g, w, Nb)
    elif geom == 'ipchain':
        H, l = setup_ipchain_hamiltonian(eps, delta*renorm, g, w, Nb)
    else:
        H, l = setup_star_hamiltonian(eps, delta*renorm, g, w, Nb)
...
if __name__ == "__main__":
    ...
    sbm_dynamics(210, args.alpha, args.wc, args.s, args.delta, args.eps, args.chi, args.nbose, args.dt, nstep = nstep, geom=args.geom, ofname = args.fname, beta=args.beta)
