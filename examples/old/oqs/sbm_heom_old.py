...
def sbm_dynamics(alpha, wc, s, eps, delta, chi, nbose, dt, nbose_min = None, beta = None, Ncut = 20, nstep = 1, Nw = 20, geom='star', ofname='sbm.h5', degree = 2, adaptive=True, spawning_threshold=2e-4, unoccupied_threshold=1e-4, nunoccupied=0, aaa_tol=1e-4):
    ...
    Hsys = eps*sz + 0.5*delta *sx
    H += sOP("lsys", 0)

    for i in range(Nb):
        H += sOP("sp", 0)*sOP("bp", i+1)
        if i % 2 == 0:
            H += sOP("sm_f", 0)*sOP("bm", i+1)
        else:
            H += sOP("sm_b", 0)*sOP("bm", i+1)
        H += sOP("nop", i+1)
    ...
    for i in range(len(dk)):
        ...
        opdict.insert(i1+1, "bp", site_operator(csr_matrix_complex(oqs.heom.bkp(b_mode_dims[i1], dk[i]), nrows=b_mode_dims[i1], ncols=b_mode_dims[i1]), optype="sparse_matrix", mode=i1+1))
        opdict.insert(i1+1, "bm", site_operator(csr_matrix_complex(oqs.heom.bkm(b_mode_dims[i1], dk[i], True), nrows=b_mode_dims[i2], ncols=b_mode_dims[i2]), optype="sparse_matrix", mode=i1+1))
        opdict.insert(i1+1, "nop", site_operator(diagonal_matrix_complex(oqs.heom.nop(b_mode_dims[i1], zk[i])), optype="diagonal_matrix", mode=i1+1))

        opdict.insert(i2+1, "bp", site_operator(csr_matrix_complex(oqs.heom.bkp(b_mode_dims[i2], np.conj(dk[i])), nrows=b_mode_dims[i1], ncols=b_mode_dims[i1]), optype="sparse_matrix", mode=i2+1))
        opdict.insert(i2+1, "bm", site_operator(csr_matrix_complex(oqs.heom.bkm(b_mode_dims[i2], np.conj(dk[i]), False), nrows=b_mode_dims[i2], ncols=b_mode_dims[i2]), optype="sparse_matrix", mode=i2+1))
        opdict.insert(i2+1, "nop", site_operator(diagonal_matrix_complex(oqs.heom.nop(b_mode_dims[i2], np.conj(zk[i]))), optype="diagonal_matrix", mode=i2+1))

    #setup the system information object
    sysinf = system_modes(N)
    for i in range(N):
        sysinf[i] = generic_mode(mode_dims[i])
