...
def hamiltonian(B, g, A, ks, kt):
    ...
    for i in range(Nb):
        H += A[i] * (sOP("sx", 1) * sOP("sx", i+2) )
        H += A[i] * (sOP("sy", 1) * sOP("sy", i+2) )
        H += A[i] * (sOP("sz", 1) * sOP("sz", i+2) )
...
csm_test(49, 0.0, 16, 0.025, nstep = 4000, yrange =[-0.5, 0.5])
