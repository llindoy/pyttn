...
def xytree_dynamics(Nl, Nb, alpha, wc, eta, chi, chiS, chiB, nbose, dt,  
                    nbose_min=None, Ecut = 10, beta=None, nstep=1, Nw=4.0, geom="ipchain", ofname="xytree.h5", 
                    degree=2, adaptive=True, spawning_threshold=2e-4, unoccupied_threshold=1e-4, 
                    nunoccupied=0, nbmax=2, nhilbmax=1024):
    ...
    for ind in hiterms:
        skip1 = (ind[0]) * (Nb + 1)
        skip2 = (ind[1]) * (Nb + 1)
        H += (1 - eta) * pyttn.sOP("sx", skip1) * pyttn.sOP("sx", skip2) + (1 + eta) * pyttn.sOP("sy", skip1) * pyttn.sOP("sy", skip2)
    ...
    topo = build_topology(Nl, 2, chi0, chiS0, chiB0, nbose, discbath, degree)
    capacity = build_topology(Nl, 2, chi, chiS, chiB, nbose, discbath, degree)

    print("topology built")

    # construct and initialise the ttn wavefunction
    A = pyttn.ttn(topo, capacity, dtype=np.complex128)
    #set up the spin excitation on just the central site of the tree (0)
...
if __name__ == "__main__":
    ...
    xytree_dynamics(args.Nl, args.N, args.alpha, args.wc, args.eta, args.chi, args.chiS, args.chiB, args.nbose, args.dt, 
