...
def xychain_dynamics(
    ...
    for si in range(Ns):
        ...
        if si + 1 != Ns:
            skip2 = (si + 1) * (Nb + 1)
            H += (1 - eta) * sOP("sx", skip) * sOP("sx", skip2) + (1 + eta) * sOP(
                "sy", skip
            ) * sOP("sy", skip2)
        ...
        if Nb != 0:
            ...
            H, frequencies = oqs.unitary.add_bosonic_bath_hamiltonian(
                H,
                sOP("sz", skip),
                g,
                w,
    ...
    if Nb != 0:
        ...
        b_mode_dims = [
            min(max(4, int(wc * 20 / frequencies[i])), nbose) for i in range(Nb)
        ]
