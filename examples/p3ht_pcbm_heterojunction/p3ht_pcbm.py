...
def build_ttn_tree(mdims, chi1, chi2, ldims, degree=2, use_multiset=False):
    ...
    if(use_multiset):
        ...
        ntreeBuilder.mlmctdh_subtree(topo()[2][0], mdims[modeOT_lf[:]], 2, chi_step(chi1, chim1, len(modeOT_lf)), ldims[modeOT_lf[:]])
        ntreeBuilder.mlmctdh_subtree(topo()[2][1], mdims[modeOT_hf[:]], 2, chi_step(chi2, chim2, len(modeOT_hf)), ldims[modeOT_hf[:]])
...
def p3ht_pcbm_dynamics(ansatz, chi1, chi2=None, tmax=200, dt=0.25, use_multiset=False, adaptive=True, spawning_threshold=1e-6, unoccupied_threshold=1e-4, nunoccupied=0, ofname='p3ht_pcbm.h5', output_skip=1):
