...
def build_topology(Ns, ds, chi, chiS, chiB, nbose, b_mode_dims, degree):
    ...
    for li in leaf_indices:
        ...
        if len(b_mode_dims) != 0:
            if degree == 1:
                ntreeBuilder.mps_subtree(
                    topo.at(li), b_mode_dims, chiB, min(chiB, nbose)
                )
            else:
...
def observable_tree(Ns, obstree, op, b_mode_dims):
    ...
    for i in range(Ns):
        ...
        for i in range(len(b_mode_dims)):
            state_vec = np.identity(
                int(np.sqrt(b_mode_dims[i])), dtype=np.complex128
            ).flatten()
...
def xychain_dynamics(
        ...
        if K != 0:
            ...
            for i in range(len(zk)):
                ...
                H += (
                    ...
                    * (
                        sOP("a", i1) * sOP("a", i2)
                        - 0.5 * (sOP("n", i1) + sOP("n", i2))
                    )
                )
                H += complex(Vk[i]) * (
                    sOP("sz", skip) * (sOP("adag", i1) + sOP("a", i1))
                    - sOP("sz", skip + 1) * (sOP("adag", i2) + sOP("a", i2))
                )
                H += (
                    2.0j
                    * complex(Mk[i])
                    * (
                        sOP("sz", skip + 1) * sOP("a", i1)
                        - 0.5
                        * (
                            sOP("sz", skip) * sOP("a", i1)
                            + sOP("sz", skip + 1) * sOP("adag", i2)
                        )
                    )
                )
                H += (
                    2.0j
                    * complex(np.conj(Mk[i]))
                    * (
                        sOP("sz", skip) * sOP("a", i2)
                        - 0.5
                        * (
                            sOP("sz", skip) * sOP("adag", i1)
                            + sOP("sz", skip + 1) * sOP("a", i2)
                        )
                    )
                )
    ...
    topo = build_topology(
        Ns, sysinfo[0].lhd(), chi0, chiS0, chiB0, L, b_mode_dims, degree
    )
    capacity = build_topology(
        Ns, sysinfo[0].lhd(), chi, chiS, chiB, L, b_mode_dims, degree
    )
