...
def xychain_dynamics(
    ...
    @jit(nopython=True)
    def J(w):
        return np.abs(2 * np.pi * alpha * w * np.exp(-np.abs(w / wc)) ** 2) * np.where(
            w > 0, 1.0, -1.0
        )
    ...
    fig, ax = plt.subplots(
        2, 2, figsize=(3.25 * 2.5, 3.25 * 2), sharey=False, sharex=True
    )
    ...
    for i, K in enumerate(Ks):
        ...
        ax[0, 1].plot(
            t,
            oqs.BosonicBath.Ctexp(t, dk, -1.0j * zk),
            "--",
            color=colors[i],
            linewidth=3,
            label=r"$K=" + str(K) + "$",
    ...
    plt.savefig(
        "dissipative_xy_bath_discretisation_convergence.pdf", bbox_inches="tight"
    )
    plt.show()
