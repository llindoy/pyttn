...
def sbm_dynamics(
    ...
    Lsys = (eps * sOP("sz", 0) + delta * sOP("sx", 0)) - (
        eps * sOP("sz", 1) + delta * sOP("sx", 1)
    )
    H += Lsys
    ...
    for i in range(Nb):
        H += -1.0j * zk[i] * sOP("n", i + 2)
        H += complex(dk[i]) * (sOP("sz", 0) - sOP("sz", 1)) * sOP("a", i + 2)
        if i % 2 == 0:
            H += dk[i] * sOP("sz", 0) * sOP("adag", i + 2)
        else:
            H += -dk[i] * sOP("sz", 1) * sOP("adag", i + 2)
    ...
    for i in range(nstep):
        ...
        print(
            (i + 1) * dt, res[i + 1], np.real(renorm), maxchi[i + 1], np.real(mel(A, A))
        )
        sys.stdout.flush()
        if i % 10 == 0:
...
if __name__ == "__main__":
    ...
    # output file name
    parser.add_argument("--fname", type=str, default="sbm_heom.h5")
