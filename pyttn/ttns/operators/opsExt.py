...
def __sparse_matrix_cuda(*args, dtype=np.complex128):
    if (len(args) == 1):
        if isinstance(args[0], sp.sparse.csr_matrix) or isinstance(args[0], sp.sparse.coo_matrix):
            ...
            if (m2.dtype == np.complex128 or dtype == np.complex128):
                return cuops.sparse_matrix_complex(m2.data, m2.indices, m2.indptr, ncols=m2.shape[1])
            else:
                return cuops.sparse_matrix_real(m2.data, m2.indices, m2.indptr, ncols=m2.shape[1])
        else:
