...
from .bondDimensionSetter import BondDimensionSetter
...
def __setup_bond_properties_internal(node, chi, dims, level, chi_local_transform, node_list, nindex):
   if node.is_leaf():
   ...
   for i in range(node.size()):
      __setup_bond_properties_internal(node.at(i), chi, dims, level+1, chi_local_transform, node_list, nindex + [i])
...
def set_topology_properties(root, chi, dims, chi_local_transform=None):
   ...
   if isinstance(root, ntree):
      __setup_bond_properties_internal(root(), chi, dims, 0, chi_local_transform, node_list, [])
