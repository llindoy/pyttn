...
void init_operator_dictionary(py::module &m, const std::string &label)
{
     ...
     auto cls = py::class_<opdict>(m, (std::string("operator_dictionary_") + label).c_str());
     cls.def(py::init())
         .def(py::init<size_t>())
         ...
         .def("__str__", [](const opdict &o)
            ...
            return oss.str(); })
         .def("backend", [](const opdict &){ return linalg::traits<backend>::label(); });
         python_bindings::bind_pickleable(cls);
         python_bindings::bind_dtype<T>(cls);
}
