...
namespace linalg
{
    ...
    {
        __device__ bool andOp::operator()(const bool& a, const bool& b) const{return a && b;}

        template <typename T>
        __device__ T mulOp<T>::operator()(const T& a, const T& b) const{return a*b;}
        ...
        template <typename T>
        __global__ void copy_tensor3_strided(const T* __restrict__ src, T* __restrict dst, size_t D, size_t H, size_t W, size_t Hout)
        {
            size_t w = blockIdx.x * blockDim.x + threadIdx.x;   //fastest index
            size_t h = blockIdx.y * blockDim.y + threadIdx.y;
            size_t d = blockIdx.z * blockDim.z + threadIdx.z;

            if(w < W && h < H && d < D)
            {
                dst[ (d*Hout + h)*W+w] = src[ (d*H + h)*W + w];
            }
        }

        template <typename T>
        __global__ void copy_tensor3_append(const T* __restrict__ src, T* __restrict dst, size_t D, size_t H, size_t W, size_t iadd, size_t Hout)
        {
            size_t w = blockIdx.x * blockDim.x + threadIdx.x;   //fastest index
            size_t h = blockIdx.y * blockDim.y + threadIdx.y;
            size_t d = blockIdx.z * blockDim.z + threadIdx.z;
            ...
            if(w < W && h < iadd && d < D)
            {
                dst[ (d*Hout + (h + H))*W+w] = src[ (d*H + h)*W + w];
            }
        }

        template <typename T, size_t MAX_D>
            ...
            T* __restrict dst, const size_t* __restrict__ dst_strides, const size_t* __restrict__ dst_offset, size_t D, size_t nelem)
        {
            ...
            //now we go ahead and unravel the linear 1d coordinate into the D-d coordinate space
            size_t coord[MAX_D];
            size_t tmp = idx;
            for(size_t id = 0; id < D; ++id)
            {
                size_t d  = id-(D+1);
                coord[d] = tmp % src_dims[d];
                tmp /= src_dims[d];
            }
            ...
            {
                src_off += coord[d] * src_strides[d];
                dst_off += (coord[d]+dst_offset[d]) * dst_strides[d]; 
            }
...
#pragma unroll
            for (; tidx < n; tidx += stride)
            {
                out[tidx] = cuda::std::complex<T>(in[tidx], 0.0);
            }
        }

        template <typename T>
        __global__ void addition_assign_real_to_complex_array(const T *in, const size_t n, cuda::std::complex<T> *out)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        template <typename T>
        __global__ void subtraction_assign_real_to_complex_array(const T *in, const size_t n, cuda::std::complex<T> *out)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        template <typename T>
        __global__ void fill_matrix_block(const T *src, size_t m1, size_t n1, T *dest, size_t m2, size_t n2)
        {
...
#pragma unroll
        ...
        template <typename T>
        __global__ void complex_conjugate(const cuda::std::complex<T> *const in, const size_t n, cuda::std::complex<T> *const out)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
            for (; tidx < n; tidx += stride)
            {
                out[tidx] = conj(in[tidx]);
            }
        }

        template <typename T>
        __global__ void vector_scalar_product(int N, T A, const T *X, int INCX, T *Y, int INCY)
        {
...
#pragma unroll
                for (; tidx < N; tidx += stride)
                {
                    Y[tidx * INCY] = A * X[tidx * INCX];
                }
            }
        }

        template <typename T>
        __global__ void axpy_conj(const cuda::std::complex<T> *in, const size_t n, cuda::std::complex<T> *out)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        __global__ void eval_expression_strided_kernel(T *res, size_t n, size_t resstride, expr op)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
            for (; tidx < n; tidx += stride)
            {
                res[tidx * resstride] = op[tidx];
            }
        }

        template <typename T, typename expr>
        __global__ void eval_add_expression_strided_kernel(T *res, size_t n, size_t resstride, expr op)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        template <typename T, typename expr>
        __global__ void eval_sub_expression_strided_kernel(T *res, size_t n, size_t resstride, expr op)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        template <typename T, typename expr>
        __global__ void eval_expression_kernel(T *res, size_t n, expr op)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        template <typename T, typename expr>
        __global__ void eval_add_expression_kernel(T *res, size_t n, expr op)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
        ...
        template <typename T, typename expr>
        __global__ void eval_sub_expression_kernel(T *res, size_t n, expr op)
        {
            unsigned int tidx = threadIdx.x + blockDim.x * blockIdx.x;
            unsigned int stride = blockDim.x * gridDim.x;
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx] = alpha * opa(A[tidx]) * opb(B[tidx]) + beta * C[tidx];
                        }
                        else
                        {
                            C[tidx] = T5(0.0);
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx] = opa(A[tidx]) * opb(B[tidx]) + beta * C[tidx];
                        }
                        else
                        {
                            C[tidx] = T5(0.0);
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx] = alpha * opa(A[tidx]) * opb(B[tidx]);
                        }
                        else
                        {
                            C[tidx] = T5(0.0);
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx * incc] = alpha * opa(A[tidx * inca]) * opb(B[tidx * incb]) + beta * C[tidx * incc];
                        }
                        else
                        {
                            C[tidx * incc] = T5(0.0);
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx * incc] = opa(A[tidx * inca]) * opb(B[tidx * incb]) + beta * C[tidx * incc];
                        }
                        else
                        {
                            C[tidx * incc] = T5(0.0);
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx * incc] = alpha * opa(A[tidx * inca]) * opb(B[tidx * incb]);
                        }
                        else
                        {
                            C[tidx * incc] = T5(0.0);
...
#pragma unroll
                    for (; tidx < n; tidx += stride)
                    {
                        if (tidx < m)
                        {
                            C[tidx * incc] = opa(A[tidx * inca]) * opb(B[tidx * incb]);
                        }
                        else
                        {
                            C[tidx * incc] = T5(0.0);
...
#pragma unroll
            for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
            {
                if (y + j < min_km && x < n)
                {
                    blocal[(threadIdx.y + j) * sstr + threadIdx.x] = B[(y + j) * ldb + x];
                }
            }
            ...
            if (threadIdx.x == 0)
            {
                if (alpha != T1(1.0))
                {
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_km)
                        {
                            alocal[threadIdx.y + j] = alpha * opa(A[(y + j) * inca]);
                        }
                    }
                }
                else
                {
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_km)
                        {
                            alocal[threadIdx.y + j] = opa(A[(y + j) * inca]);
                        }
                    }
                }
            }
...
#pragma unroll
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_km)
                        {
                            C[(y + j) * ldc + x] = alocal[threadIdx.y + j] * opb(blocal[(threadIdx.y + j) * sstr + threadIdx.x]) + beta * C[(y + j) * ldc + x];
                        }
                        else if (y + j < m)
                        {
                            C[(y + j) * ldc + x] *= beta;
...
#pragma unroll
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_km)
                        {
                            C[(y + j) * ldc + x] = alocal[threadIdx.y + j] * opb(blocal[(threadIdx.y + j) * sstr + threadIdx.x]);
                        }
                        else if (y + j < m)
                        {
                            C[(y + j) * ldc + x] = T5(0.0);
...
#pragma unroll
            ...
            if (threadIdx.y == 0)
            {
                if (alpha != T1(1.0))
                {
                    if (x < min_km)
                    {
                        alocal[threadIdx.x] = alpha * opa(A[x * inca]);
                    }
                }
                else
                {
                    if (x < min_km)
                    {
                        alocal[threadIdx.x] = opa(A[x * inca]);
                    }
                }
            }
...
#pragma unroll
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_km)
                        {
                            C[(y + j) * ldc + x] = alocal[threadIdx.y + j] * opb(blocal[threadIdx.x * sstr + threadIdx.y + j]) + beta * C[(y + j) * ldc + x];
                        }
                        else if (y + j < m)
                        {
                            C[(y + j) * ldc + x] *= beta;
...
#pragma unroll
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_km)
                        {
                            C[(y + j) * ldc + x] = alocal[threadIdx.y + j] * opb(blocal[threadIdx.x * sstr + threadIdx.y + j]);
                        }
                        else if (y + j < m)
                        {
                            C[(y + j) * ldc + x] = T5(0.0);
...
#pragma unroll
            for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
            {
                if (y + j < m && x < min_kn)
                {
                    alocal[(threadIdx.y + j) * sstr + threadIdx.x] = A[(y + j) * lda + x];
                }
            }
            ...
            if (threadIdx.y == 0)
            {
                if (alpha != T1(1.0))
                {
                    if (x < min_kn)
                    {
                        blocal[threadIdx.x] = alpha * opb(B[x * incb]);
                    }
                }
                else
                {
                    if (x < min_kn)
                    {
                        blocal[threadIdx.x] = opb(B[x * incb]);
                    }
                }
            }
...
#pragma unroll
                for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                {
                    if (y + j < m)
                    {
                        if (x < min_kn)
                        {
                            C[(y + j) * ldc + x] = blocal[threadIdx.x] * opa(alocal[(threadIdx.y + j) * sstr + threadIdx.x]) + beta * C[(y + j) * ldc + x];
                        }
                        else if (x < n)
                        {
                            C[(y + j) * ldc + x] *= beta;
...
#pragma unroll
                for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                {
                    if (y + j < m)
                    {
                        if (x < min_kn)
                        {
                            C[(y + j) * ldc + x] = blocal[threadIdx.x] * opa(alocal[(threadIdx.y + j) * sstr + threadIdx.x]);
                        }
                        else if (x < n)
                        {
                            C[(y + j) * ldc + x] = T5(0.0);
            ...
            if (threadIdx.x == 0)
            {
                if (alpha != T1(1.0))
                {
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_kn)
                        {
                            blocal[threadIdx.y + j] = alpha * opb(B[(y + j) * incb]);
                        }
                    }
                }
                else
                {
                    for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                    {
                        if (y + j < min_kn)
                        {
                            blocal[threadIdx.y + j] = opb(B[(y + j) * incb]);
                        }
                    }
                }
            }
...
#pragma unroll
                for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                {
                    if (y + j < m)
                    {
                        if (x < min_kn)
                        {
                            C[(y + j) * ldc + x] = blocal[threadIdx.x] * opa(alocal[threadIdx.x * sstr + threadIdx.y + j]) + beta * C[(y + j) * ldc + x];
                        }
                        else if (x < n)
                        {
                            C[(y + j) * ldc + x] *= beta;
...
#pragma unroll
                for (size_t j = 0; j < TILE_DIM; j += BLOCK_ROWS)
                {
                    if (y + j < m)
                    {
                        if (x < min_kn)
                        {
                            C[(y + j) * ldc + x] = blocal[threadIdx.x] * opa(alocal[threadIdx.x * sstr + threadIdx.y + j]);
                        }
                        else if (x < n)
                        {
                            C[(y + j) * ldc + x] = T5(0.0);
