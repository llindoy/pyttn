...
#include <linalg/backends/cuda/cuda_kernels.cuh>
...
namespace linalg
{
    namespace cuda_internals
    {
        ...
        template <>
        struct ones_indexer<std::complex<float>>
        {
            static inline constexpr size_t index() { return 2; }
        };
        template <>
        struct ones_indexer<std::complex<double>>
        {
    ...
    /*
     *  Functions for handling the ones buffers
     */
    cuda_backend::ones_state &cuda_backend::ones()
    {
        static ones_state s;
        return s;
    }
    ...
    template <typename T>
    void cuda_backend::initialise_empty_ones_buffer()
    {
        ones_type<T> &_ones = std::get<cuda_internals::ones_indexer<T>::index()>(ones());
    ...
    {
        clean_up_ones();
        initialise_empty_ones_buffer<float>();
        initialise_empty_ones_buffer<double>();
        initialise_empty_ones_buffer<std::complex<float>>();
        initialise_empty_ones_buffer<std::complex<double>>();
    }
    ...
    {
        clean_up_ones<float>();
        clean_up_ones<double>();
        clean_up_ones<std::complex<float>>();
        clean_up_ones<std::complex<double>>();
    }

    template <typename T>
    void cuda_backend::allocate_ones(size_type n)
    {
        try
        {
    ...
    template <typename T>
    bool cuda_backend::is_equal(const T *a, const T *b, size_type N)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, void>::type cuda_backend::vector_scalar_product(int_type N, T A, const T *X, int_type INCX, T *Y, int_type INCY)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, T>::type cuda_backend::trace(int_type N, const T *X, int_type INCX)
    {
        ASSERT(environment().is_initialised(), "cuda backend vector_scalar_product call failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((N + nthreads - 1) / nthreads);
    ...
    // valid blas routines
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, void>::type cuda_backend::axpy(int_type N, T A, const T *X, int_type INCX, T *Y, int_type INCY)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, void>::type cuda_backend::scal(int_type N, T A, T *X, int_type INCX)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, T>::type cuda_backend::dot(bool conj, int_type N, const T *X, int_type INCX, const T *Y, int_type INCY)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, void>::type
    cuda_backend::gemm(transform_type TRANSA, transform_type TRANSB, int_type M, int_type N, int_type K, T ALPHA, const T *A, int_type LDA, const T *B, int_type LDB, T BETA, T *C, int_type LDC)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, void>::type
    cuda_backend::gemv(transform_type trans, int_type m, int_type n, T alpha, const T *A, int_type lda, const T *x, int_type incx, T beta, T *y, int_type incy)
    {
    ...
    template <typename T>
    typename std::enable_if<is_valid_value_type<T>::value, void>::type
    cuda_backend::batched_gemm(transform_type opA, transform_type opB, int_type m, int_type n, int_type k, T alpha, const T *A, int_type lda, long long int strideA, const T *B, int_type ldb, long long int strideB, T beta, T *C, int_type ldc, long long int strideC, int_type batchCount)
    {
    ...
    template <typename T>
    void cuda_backend::outer_contract(transform_type opA, transform_type opB, int_type m, int_type n, int_type k, T alpha, const T *A,
                                             ...
                                             long long int strideC, int_type batchCount, T *res)
    {
        ...
        CALL_AND_HANDLE(batched_gemm(map_op(opA), map_op(opB), m, n, k, alpha, A, lda, strideA, B, ldb, strideB, b, C, ldc, strideC, batchCount), "Failed to compute contraction of outer indices of two rank three tensors.  Failed to compute the contraction over the last index.");
        ...
        auto &_ones = std::get<cuda_internals::ones_indexer<T>::index()>(ones());
        ...
        int_type ldav = mv;
        CALL_AND_HANDLE(gemv(map_op(op), mv, nv, a, C, ldav, std::get<0>(_ones), incx, beta, res, incy), "Failed to compute contraction of outer indices of two rank three tensor.  Failed to compute the contraction over the first index.");
    }

    // sparse matrix vector operations
    template <typename T1, typename T2, typename T3, typename T4, typename T5>
    void cuda_backend::dgmv(bool conjA, bool conjB, int_type m, int_type n, T1 alpha, const T2 *A, int_type inca, const T3 *X, int_type incx, T4 beta, T5 *Y, int_type incy)
    {
    ...
    template <typename T1, typename T2, typename T3, typename T4, typename T5>
    void cuda_backend::dgmm(bool sparse_left, transform_type opA, transform_type opB, size_type m, size_type n, size_type k, T1 alpha, const T2 *A, size_type inca, const T3 *B, size_type ldb, T4 beta, T5 *C, size_type ldc)
    {
    ...
    template <typename T1, typename T2, typename T3, typename T4, typename T5>
    void cuda_backend::dgmv_kernel_selector(bool conjA, bool conjB, size_type m, size_type n, T1 alpha, const T2 *A, int_type inca, const T3 *X, int_type incx, T4 beta, T5 *Y, int_type incy)
    {
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
    ...
    template <typename T1, typename T2, typename T3, typename T4, typename T5>
    void cuda_backend::dgmm_kernel_parameter_selector(bool sparse_left, transform_type opA, transform_type opB, size_type m, size_type n, size_type k, T1 alpha, const T2 *A, size_type inca, const T3 *B, size_type ldb, T4 beta, T5 *C, size_type ldc)
    {
    ...
    void cuda_backend::dgmm_kernel_launcher(bool sparse_left, transform_type opA, transform_type opB, size_type m, size_type n, size_type k, T1 alpha, const T2 *A, size_type inca, const T3 *B, size_type ldb, T4 beta, T5 *C, size_type ldc)
    {
        bool conjA = false;
        bool conjB = false;
    ...
    template <typename T>
    void cuda_backend::csrmv(transform_type opA, bool /*conjB*/, int_type m, int_type n, size_type nnz, T alpha, const T *A, const int *rowptr, const int *colind, const T *X, int_type incx, T beta, T *Y, int_type incy)
    {
    ...
    template <typename T>
    void cuda_backend::csrmv(transform_type opA, bool conjB, int_type m, int_type n, size_type nnz, std::complex<T> alpha, const std::complex<T> *A, const int *rowptr, const int *colind, const std::complex<T> *X, int_type incx, std::complex<T> beta, std::complex<T> *Y, int_type incy)
    {
    ...
    template <typename T>
    void cuda_backend::csrmm(bool opres, transform_type opA, transform_type opB, size_type m, size_type n, size_type k, size_type nnz, T alpha, const T *A, const int *rowptr, const int *colind, const T *B, size_type ldb, T beta, T *C, size_type ldc)
    {
    ...
    template <typename T>
    void cuda_backend::complex_conjugate(size_type /*size*/, const T *const /*X*/, T *const /*Y*/) {}

    template <typename T>
    void cuda_backend::complex_conjugate(size_type size, const std::complex<T> *const X, std::complex<T> *const Y)
    {
    ...
    void cuda_backend::evaluate_expression_tree(T *res, size_type n, const expr &e)
    {
    ...
    template <typename T, typename expr>
    void cuda_backend::evaluate_add_expression_tree(T *res, size_type n, const expr &e)
    {
        ASSERT(environment().is_initialised(), "cuda backend evaluate expression tree call failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T, typename expr>
    void cuda_backend::evaluate_sub_expression_tree(T *res, size_type n, const expr &e)
    {
        ASSERT(environment().is_initialised(), "cuda backend evaluate expression tree call failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T, typename expr>
    void cuda_backend::evaluate_expression_tree_strided(T *res, size_type n, size_type stride, const expr &e)
    {
        ASSERT(environment().is_initialised(), "cuda backend evaluate expression tree call failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T, typename expr>
    void cuda_backend::evaluate_add_expression_tree_strided(T *res, size_type n, size_type stride, const expr &e)
    {
        ASSERT(environment().is_initialised(), "cuda backend evaluate expression tree call failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T, typename expr>
    void cuda_backend::evaluate_sub_expression_tree_strided(T *res, size_type n, size_type stride, const expr &e)
    {
        ASSERT(environment().is_initialised(), "cuda backend evaluate expression tree call failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T>
    void cuda_backend::transpose(bool conj, int_type m, int_type n, const T &alpha, const T *in, const T &beta, T *out)
    {
        ...
        CALL_AND_HANDLE(cublas::geam(hCublas(), map_op(op), transform_type::n, m, n, &alpha, in, n, &beta, in, m, out, m), "Failed to evaluate cuda_backend::transpose.  Error when calling geam.");
    }
    ...
    template <typename T>
    void cuda_backend::batched_transpose(bool conj, size_type m, size_type n, const T &alpha, const T *in, const T &beta, T *out, size_type batchCount)
    {
        ...
        for (size_type i = 0; i < batchCount; ++i)
        {
            ...
            CALL_AND_HANDLE(cublas::geam(hCublas(), map_op(op), transform_type::n, m, n, &alpha, in, n, &beta, in + bskip, m, out + bskip, m), "Failed to evaluate cuda_backend::transpose.  Error when calling geam.");
    ...
    template <typename T>
    void cuda_backend::copy(const T *src, size_type n, T *dest)
    {
    ...
    template <typename T>
    void cuda_backend::rank_3_strided_copy(const T *src, size_type n1, size_type n2, size_type n3, T *dest, size_type n4)
    {
    ...
    template <typename T>
    void cuda_backend::rank_3_strided_append(const T *src, size_type n1, size_type n2, size_type n3, size_type iadd, T *dest, size_type n4)
    {
        ASSERT(environment().is_initialised(), "cuda backend addition_assign_real_to_complex operation failed.  The cuda environment has not yet been initialised.");
        size_t Bx = 32;
        size_t By = 4;
    ...
    template <typename T>
    void cuda_backend::assign(const T *src, size_type n, T *dest, T beta)
    {
        ASSERT(environment().is_initialised(), "cuda backend addition_assign_real_to_complex operation failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T>
    void cuda_backend::addition_assign(const T *src, size_type n, T *dest)
    {
        ASSERT(environment().is_initialised(), "cuda backend addition_assign_real_to_complex operation failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T>
    void cuda_backend::subtraction_assign(const T *src, size_type n, T *dest)
    {
        ASSERT(environment().is_initialised(), "cuda backend addition_assign_real_to_complex operation failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T>
    void cuda_backend::copy_real_to_complex(const T *src, size_type n, std::complex<T> *dest)
    {
    ...
    template <typename T>
    void cuda_backend::addition_assign_real_to_complex(const T *src, size_type n, std::complex<T> *dest)
    {
        ASSERT(environment().is_initialised(), "cuda backend addition_assign_real_to_complex operation failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T>
    void cuda_backend::subtraction_assign_real_to_complex(const T *src, size_type n, std::complex<T> *dest)
    {
        ASSERT(environment().is_initialised(), "cuda backend addition_assign_real_to_complex operation failed.  The cuda environment has not yet been initialised.");
        size_type nthreads = environment().maximum_dimensions_threads_per_block()[0];
        dim3 dg((n + nthreads - 1) / nthreads);
    ...
    template <typename T>
    void cuda_backend::copy_matrix_subblock(size_type m1, size_type n1, const T *src, size_type lda, T *dest, size_type ldb)
    {
    ...
    template <typename T>
    void cuda_backend::fill_n(T *dest, size_type n, const T &val)
    {
    ...
    void cuda_backend::func_fill_1(T *res, size_type m, Func &&f, Args &&...args)
    {
    ...
    template <typename T, typename Func, typename... Args>
    void cuda_backend::func_fill_2(T *res, size_type m, size_type n, Func &&f, Args &&...args)
    {
    ...
    template <typename T, typename Func, typename... Args>
    void cuda_backend::func_fill_3(T *res, size_type m, size_type n, size_type o, Func &&f, Args &&...args)
    {
    ...
    template <typename T>
    void cuda_backend::fill_matrix_block(const T *src, size_type m, size_type n, T *dest, size_type m2, size_type n2)
    {
    ...
    void cuda_backend::set_tensor_block(const T *src, const std::array<size_type, D> &src_dims, T *dest, const std::array<size_type, D> &dest_dims, const std::array<size_type, D> &skip)
    {
        ASSERT(environment().is_initialised(), "cuda backend fill_n operation failed.  The cuda environment has not yet been initialised.");
        ...
        cuda_kernels::copy_tensor_subblock_nd<<<dg, db, 0, hCuda()>>>(src, &src_stride, &src_dims, dest, &dest_stride, &skip, D, n);    
    }

    template <typename T>
    void cuda_backend::transfer_coo_tuple_to_csr(const std::vector<std::tuple<index_type, index_type, T>> &coo, T *vals, index_type *colinds)
    {
    ...
    template <typename T>
    void cuda_backend::heev(eig_mode jobz, fill_mode uplo, int_type n, T *A, int_type lda, typename linalg::get_real_type<T>::type *W, T *work, int_type lwork, int *devinfo)
    {
    ...
    template <typename T>
    void cuda_backend::heev_buffersize(eig_mode jobz, fill_mode uplo, int_type n, T *A, int_type lda, typename linalg::get_real_type<T>::type *W, int *lwork)
    {
        ASSERT(environment().is_initialised(), "cuda backend heev call failed.  The cuda environment has not yet been initialised.");
    ...
    template <typename T>
    void cuda_backend::getrf(int_type m, int_type n, T *A, int_type lda, T *work, int *ipiv, int *devinfo)
    {
    ...
    template <typename T>
    void cuda_backend::getrf_buffersize(int_type m, int_type n, T *A, int_type lda, int *lwork)
    {
        ASSERT(environment().is_initialised(), "cuda backend getrf call failed.  The cuda environment has not yet been initialised.");
    ...
    template <typename T>
    void cuda_backend::gesvd(const char jobu, const char jobv, const int_type m, const int_type n, T *A, const int_type lda, typename linalg::get_real_type<T>::type *S, T *U, const int_type ldu, T *VT, const int_type ldvt, T *work, const int_type lwork, typename linalg::get_real_type<T>::type *rwork, int *devinfo)
    {
        ASSERT(environment().is_initialised(), "cuda backend heev call failed.  The cuda environment has not yet been initialised.");
    ...
    template <typename T>
    void cuda_backend::gesvd_buffersize(int_type m, int_type n, int &lwork)
    {
        ASSERT(environment().is_initialised(), "cuda backend heev call failed.  The cuda environment has not yet been initialised.");
    ...
    template <typename T>
    void cuda_backend::gesvdj_buffersize(eig_mode jobz, const int_type econ, const int_type m, const int_type n, T *A, const int_type lda, typename linalg::get_real_type<T>::type *S, T *U, const int_type ldu, T *VT, const int_type ldvt, int &lwork, void* params)
    {
        ASSERT(environment().is_initialised(), "cuda backend heev call failed.  The cuda environment has not yet been initialised.");
    ...
    template <typename T>
    void cuda_backend::gesvdj(eig_mode jobz, const int_type econ, const int_type m, const int_type n, T *A, const int_type lda, typename linalg::get_real_type<T>::type *S, T *U, const int_type ldu, T *VT, const int_type ldvt, T *work, const int_type lwork, int *devinfo, void* params)
    {
        ASSERT(environment().is_initialised(), "cuda backend heev call failed.  The cuda environment has not yet been initialised.");
    ...
    void cuda_backend::tensor_transpose(const T *in, const arr1 &dimsA, const arr2 &_strideA, T *out, const arr3 &dimsB, const arr4 &_strideB, const std::vector<size_type> &inds)
    {
    ...
    template <typename T>
    T cuda_backend::determinant_reduction(T *red, size_t N)
    {
        ASSERT(environment().is_initialised(), "cuda backend is_equal call failed.  The cuda environment has not yet been initialised.");
