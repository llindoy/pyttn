...
void initialise_dmrg_adaptive_ttn_cuda(py::module &m)
{
    using complex_type = std::complex<pyttn_real_type>;
    init_dmrg_adaptive<complex_type, ttns::ttn, linalg::cuda_backend>(m, std::string("adaptive_one_site_dmrg_complex"));
#ifdef BUILD_REAL_TTN
    init_dmrg_adaptive<pyttn_real_type, ttns::ttn, linalg::cuda_backend>(m, std::string("adaptive_one_site_dmrg_real"));
#endif
}