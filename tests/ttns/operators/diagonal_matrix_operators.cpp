...
TEMPLATE_TEST_CASE("DiagonalMatrixOperator", "[template][operator]", double, std::complex<double>)
{
    ...
    linalg::diagonal_matrix<value_type> mat = init_random<value_type>::diagonal_matrix(size, size, rng);
    //setup the identity operator
    using mat_op = ops::diagonal_matrix_operator<value_type>;
