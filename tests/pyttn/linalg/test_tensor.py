...
from pyttn.linalg.tensorExt import vector, matrix, tensor_3, tensor_4, tensor, available_backends
import pytest
import numpy as np
import os
...
def test_vector(N):
    for backend in available_backends():
        def run_test(dtype):
            ...
            m2 = tensor(m1, dtype=dtype, backend=backend)
            ...
            m3 = vector(m1, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 1
            assert m3.shape(0) == N

            m3 = vector(m2, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 1
            assert m3.shape(0) == N
...
def test_matrix(M, N):
    for backend in available_backends():
        ...
        def run_test(dtype):
            ...
            m3 = matrix(m1, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 2
            assert m3.shape(0) == M
            assert m3.shape(1) == N

            m3 = matrix(m2, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 2
            assert m3.shape(0) == M
            assert m3.shape(1) == N            
...
def test_tensor_3(M, N, L):
    for backend in available_backends():
        ...
        def run_test(dtype):
            ...
            m3 = tensor_3(m1, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 3
            assert m3.shape(0) == M
            assert m3.shape(1) == N
            ...
            m3 = tensor_3(m2, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 3
            assert m3.shape(0) == M
            assert m3.shape(1) == N
            ...
            m3 = tensor_3(m2, dtype=np.complex128)
            assert m3.complex_dtype()
            assert m3.ndim() == 3
            assert m3.shape(0) == M
            assert m3.shape(1) == N
...
def test_tensor_4(M, N, L, P):
    for backend in available_backends():
        ...
        def run_test(dtype):
            ...
            m3 = tensor_4(m1, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 4
            assert m3.shape(0) == M
            assert m3.shape(1) == N
            ...
            m3 = tensor_4(m2, dtype=np.complex128, backend=backend)
            assert m3.complex_dtype()
            assert m3.ndim() == 4
            assert m3.shape(0) == M
            assert m3.shape(1) == N
            ...
            m3 = tensor_4(m2, dtype=np.complex128)
            assert m3.complex_dtype()
            assert m3.ndim() == 4
            assert m3.shape(0) == M
            assert m3.shape(1) == N
