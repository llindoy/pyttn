...
#include "mpo_nodes/httensor_node.hpp"
...
{
public:
    static_assert(is_number<T>::value, "The first template argument to the mpo object must be a valid number type.");
    static_assert(guarenteed_valid_backend<backend>::value, "The second template argument to the mpo object must be a valid backend.");
...
public:
    mpo() {}
    mpo(size_type N) : m_nodes(N) {}
    mpo(size_type N, size_type d, size_type chi)
    {
        CALL_AND_HANDLE(resize(N, d, chi), "Failed to construct mpo object.");
    }

    template <typename U, typename be>
    mpo(const mpo<U, be>& other)
    {
        this->operator=(other);
    }

    template <typename dtype, typename chitype>
    mpo(const dtype& d, const chitype& chi)
    {
        CALL_AND_HANDLE(resize(d, chi), "Failed to construct mpo object.");
    }

    template <typename U, typename be>
    mpo& operator=(const mpo<U, be>& other)
    {
    ...
    {
        CALL_AND_HANDLE(clear(), "Failed to resize mpo object.  Failed to clear currently allocated data.");
        try
        {
            if(N > 0)
            {
    ...
    void resize(const dtype& d, const chitype& chi)
    {
        ...
        {
            size_type N = d.size();
            ASSERT(chi.size() + 1 == N, "Failed to resize mpo, bond dimension array and local hilbert space dimension arrays do not have compatible sizes.");
    ...
    void clear()
    {
        ...
        {
            std::cerr << ex.what() << std::endl;
            RAISE_EXCEPTION("Failed to clear mpo object.");
        }
    }
    ...
    void set_orthogonality_centre(size_type index)
    {
        ...
        {
            std::cerr << ex.what() << std::endl;
            RAISE_EXCEPTION("Failed to set orthogonality centre for mpo.");
        }
    }
    ...
    void right_orthogonalise()
    {
        ...
        {
            std::cerr << ex.what() << std::endl;
            RAISE_EXCEPTION("Failed to orthogonalise general mpo.");
        }
    }
...
public:
    ...
    {
        CALL_AND_HANDLE(ar(cereal::make_nvp("nodes", m_nodes)), "Failed to serialise mpo object.  Failed to serialise its mode_dimensions.");
        CALL_AND_HANDLE(ar(cereal::make_nvp("mode_dimensions", m_dim_sizes)), "Failed to serialise mpo object.  Failed to serialise its mode_dimensions.");
        CALL_AND_HANDLE(ar(cereal::make_nvp("leaf_indices", m_leaf_indices)), "Failed to serialise mpo object.  Failed to serialise its leaf_indices.");
        CALL_AND_HANDLE(ar(cereal::make_nvp("orthogonality_centre", m_orthogonality_centre)), "Failed to seriesalise mpo object. Failed to serialise orthogonality centre.");
        CALL_AND_HANDLE(ar(cereal::make_nvp("has_orthogonality_centre", m_has_orthogonality_centre)), "Failed to seriesalise mpo object. Failed to serialise orthogonality centre.");
    }
...
using mpo_node = typename linalg::tensor<T, backend, 3>;

template <typename T, typename backend> 
std::ostream& operator<<(std::ostream& os, const mpo<T, backend>& t)
{
    return os;
}
