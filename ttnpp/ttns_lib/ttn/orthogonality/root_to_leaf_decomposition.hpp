...
#ifndef PYTTN_TTNS_LIB_TTN_ORTHOGONALITY_ROOT_TO_LEAF_DECOMPOSITION_HPP_
#define PYTTN_TTNS_LIB_TTN_ORTHOGONALITY_ROOT_TO_LEAF_DECOMPOSITION_HPP_
    ...
    namespace orthogonality
    {
        ...
        public:
            ...
            static inline std::array<size_type, 2> maximum_matrix_dimension_node(const hdata &a, bool use_capacity = false)
            {
                ...
                {
                    if (a.dim(mode, use_capacity) > max_dim)
                    {
                        max_dim = a.dim(mode, use_capacity);
                    }
                }
                ret[0] /= max_dim;
                ret[1] *= max_dim;
            ...
            static inline size_type maximum_work_size_node(engine &eng, const hdata &A, mat &U, mat &R, bool use_capacity = false)
            {
                ...
                if (A.nmodes() != 1)
                {
                    for (size_type mode = 0; mode < A.nmodes(); ++mode)
                    {
                        int d1 = A.hrank(use_capacity) * A.dimen(use_capacity) / A.dim(mode, use_capacity);
                        int d2 = A.dim(mode, use_capacity);
                        ...
                        if (work_size > max_work_size)
                        {
                            max_work_size = work_size;
                        }
                    }
                }
            ...
            static inline void resize_r_matrix(const hdata &a, mat &r, bool use_capacity = false)
            {
                ...
                if (a.nmodes() > 1)
                {
                    for (size_type mode = 0; mode < a.nmodes(); ++mode)
                    {
                        max_dim = max_dim < a.dim(mode, use_capacity) ? a.dim(mode, use_capacity) : max_dim;
                    }
                }
...
#endif // PYTTN_TTNS_LIB_TTN_ORTHOGONALITY_ROOT_TO_LEAF_DECOMPOSITION_HPP_
