...
#ifndef PYTTN_TTNS_LIB_TTN_MS_TTN_HPP_
#define PYTTN_TTNS_LIB_TTN_MS_TTN_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        ms_ttn(const ms_ttn<U, backend> &other)
        try : base_type(other) {}
        catch (const std::exception &ex)
        {
            std::cerr << ex.what() << std::endl;
    ...
    public:
        ...
        void set_slice(size_type sind, const ttn<U, be> &o)
        {
            ...
            {
                if (!std::get<0>(z)()[sind].can_fit_node(std::get<1>(z)()))
                {
                    all_fit = false;
                }
            }
        ...
        void set_slice(size_type sind, const multiset_ttn_slice<U, be, C2> &o)
        {
            ...
            {
                if (!std::get<0>(z)()[sind].can_fit_node(std::get<1>(z)()[o.slice_index()]))
                {
                    all_fit = false;
                }
            }
    ...
    std::ostream &operator<<(std::ostream &os, const ms_ttn<T, backend> &t)
    {
        ...
        os << std::endl
           << static_cast<const tree<multiset_node_data<T, backend>> &>(t);
        return os;
    }
...
#endif // PYTTN_TTNS_LIB_TTN_MS_TTN_HPP_//
