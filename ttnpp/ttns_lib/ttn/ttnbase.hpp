...
#ifndef PYTTN_TTNS_LIB_TTN_TTN_BASE_HPP_
#define PYTTN_TTNS_LIB_TTN_TTN_BASE_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        template <template <typename, typename> class nc, typename U, typename be>
    ...
    protected:
        ...
        void _assign_ttn(const ttn_base<node_class, U, be> &other)
        {
            ...
            if (has_same_structure(*this, other) && other.mode_dimensions() == this->mode_dimensions() && this->nset() == other.nset() && this->is_purification() == other.is_purification())
            {
                ...
                {
                    if (!std::get<0>(z).can_fit_node(std::get<1>(z)()))
                    {
                        all_fit = false;
                    }
                }
    ...
    public:
        ...
        {
            if (m_dim_sizes.size() == 0)
            {
                return 0;
            }
            size_type nelems = 1;
            for (size_type i = 0; i < m_dim_sizes.size(); ++i)
            {
                nelems *= m_dim_sizes[i];
            }
    ...
    public:
        ...
        value_type &site_tensor(size_type lid)
        {
            ...
            if (m_has_orthogonality_centre && m_orthogonality_centre != index)
            {
                m_has_orthogonality_centre = false;
            }
        ...
        node_reference operator[](size_type i)
        {
            if (m_has_orthogonality_centre)
            {
                m_has_orthogonality_centre = (i == m_orthogonality_centre);
            }
            return m_nodes[i];
        }
    ...
    public:
        ...
        size_t path(size_t ind_i, size_t ind_j, std::list<size_type> &path)
        {
            ...
            if (ind_i != ind_j)
            {
                ...
                if (!continue_iter && jt != index_j.begin())
                {
                    --jt;
                }
                ...
                for (size_type i = 0; i < n_to_remove; ++i)
                {
                    path.push_back(0);
                }
    ...
    protected:
        ...
        void construct_topology(const ntree<INTEGER, Alloc> &__tree, const ntree<INTEGER, Alloc> &_capacity, size_type nset = 1, bool collapse_bond_matrices = true)
        {
            ...
            if (m_nodes.size() != required_size)
            {
                m_nodes.resize(required_size);
            }
...
#endif // PYTTN_TTNS_LIB_TTN_TTN_BASE_HPP_//
