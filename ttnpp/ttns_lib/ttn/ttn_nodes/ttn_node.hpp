...
#ifndef PYTTN_TTNS_LIB_TTN_TTN_NODES_TTN_NODE_HPP_
#define PYTTN_TTNS_LIB_TTN_TTN_NODES_TTN_NODE_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        {
            if (max_hrank() < o.max_hrank())
            {
                return false;
            }
            if (max_dims().size() != o.max_dims().size())
            {
                return false;
            }
            for (size_type i = 0; i < max_dims().size(); ++i)
            {
                if (max_dim(i) < o.max_dim(i))
                {
                    return false;
                }
            }
        ...
        void resize(size_type hrank, const std::vector<size_type> &mode_dims)
        {
            ...
            {
                if (m_mode_capacity[i] < mode_dims[i])
                {
                    m_mode_capacity[i] = mode_dims[i];
                }
                ndimen *= mode_dims[i];
            ...
            if (ndimen > m_max_dimen)
            {
                m_max_dimen = ndimen;
            }
            if (hrank > m_max_hrank)
            {
                m_max_hrank = hrank;
            }
        }
        ...
        size_type dim(size_type n, bool use_max_dim = false) const
        {
            if (!use_max_dim)
            {
                return m_mode_dims[n];
            }
            else
            {
                return m_mode_capacity[n];
            }
        }
        ...
        linalg::reinterpreted_tensor<T, 1, backend> as_rank_1(bool use_max_dim = false)
        {
            if (!use_max_dim)
            {
                return this->reinterpret_shape(dimen() * hrank());
            }
            else
            {
                return this->reinterpret_capacity(dimen(use_max_dim) * hrank(use_max_dim));
            }
        }
            ...
            try
            {
                ...
                if (mode < nmodes())
                {
                    if (!use_max_dim)
                    {
                        ...
                        for (size_type i = mode + 1; i < nmodes(); ++i)
                        {
                            shape[2] *= dim(i);
                        }
    ...
    public:
        ...
        void set_node_state(size_type i, linalg::random_engine<backend> &rng, bool random_unoccupied_initialisation = false)
        {
            ...
            {
                using orthog = linalg::orthogonal_vector<T, backend>;
                linalg::vector<T, linalg::blas_backend> ct(s1);
                ct.fill_zeros();
                ct(i) = T(1.0);
            ...
            {
                linalg::matrix<T, linalg::blas_backend> ct(s1, s2);
                ct.fill_zeros();
                ct(i, 0) = T(1.0);
                ...
                for (size_type j = 1; j < s2; ++j)
                {
                    ...
                    if (j % 2 == 0)
                    {
                        ...
                        {
                            RAISE_EXCEPTION("Failed to set node state unable to set unoccupied vectors.");
                        }
                    }
        ...
        void set_node_vector(const linalg::vector<U, be> &psi0, linalg::random_engine<backend> &rng)
        {
            ...
            linalg::matrix<T, backend> ct(s2, s1);
            ct.fill_zeros();
        ...
        {
            size_type _size = a.size();
            if (_size > sizes[0])
            {
                sizes[0] = _size;
            }
            size_type _capacity = a.capacity();
            if (_capacity > sizes[1])
            {
                sizes[1] = _capacity;
            }

            size_type dim2i = a.max_hrank() * a.max_hrank();
            if (dim2i > sizes[2])
            {
                sizes[2] = dim2i;
            }
...
#pragma omp parallel for num_threads(orth.nthreads()) default(shared) if (orth.parallelise() && A.nset() > 1)
    ...
    template <typename T, typename backend>
    class tree_node<tree_base<ttn_node_data<T, backend>>> : public tree_node_base<tree_base<ttn_node_data<T, backend>>>
    {
    ...
    public:
        ...
        template <template <typename, typename> class nt, typename U, typename be>
        friend class ttn_base;
        ...
        public:
            ...
            void query_sizes(tree &nodes)
            {
                ...
                for (const auto &a : nodes)
                {
                    ttn_node_data<T, backend>::query_node_sizes(a(), sizes, a.is_leaf());
                }
                m_maxsize = sizes[0];
    ...
    public:
        ...
        const ttn_node_data<T, backend> &operator()(size_type i1) const
        {
            ASSERT(i1 == 0, "Index out of bounds.");
            return m_data;
        }
        ttn_node_data<T, backend> &operator()(size_type i1)
        {
            ASSERT(i1 == 0, "Index out of bounds.");
            return m_data;
        }
    ...
    public:
        ...
        void set_interior_identity_random(size_type /*  set_index */, linalg::random_engine<backend> &rng)
        {
            ASSERT(!this->is_leaf(), "Function is only applicable for leaf state nodes.");
            this->m_data.set_node_state(0, rng, true);
        }
...
#endif // PYTTN_TTNS_LIB_TTN_TTN_NODES_TTN_NODE_HPP_//
