...
#ifndef PYTTN_TTNS_LIB_TTN_TTN_NODES_MS_TTN_NODE_HPP_
#define PYTTN_TTNS_LIB_TTN_TTN_NODES_MS_TTN_NODE_HPP_
...
namespace ttns
{
    ...
    template <typename T, typename backend>
    class tree_node<tree_base<multiset_node_data<T, backend>>> : public tree_node_base<tree_base<multiset_node_data<T, backend>>>
    {
    ...
    public:
        ...
        public:
            ...
            void clear()
            {
                for (size_type i = 0; i < m_ortho_engine.size(); ++i)
                {
                    m_ortho_engine[i].clear();
                }
            ...
            void query_sizes(tree &nodes)
            {
                ...
                for (size_type i = 0; i < nset; ++i)
                {
                    ...
                    for (const auto &a : nodes)
                    {
                        ttn_node_data<T, backend>::query_node_sizes(a()[i], sizes, a.is_leaf());
                    }
                    m_maxsize[i] = sizes[0];
    ...
    public:
        ...
        size_type maxcapacity() const
        {
            ...
            for (size_type i = 0; i < m_data.size(); ++i)
            {
                if (m_data[i].capacity() > ms)
                {
                    ms = m_data[i].capacity();
                }
            }
            return ms;
        }
        size_type maxhrank(bool use_capacity = false) const
        {
            ...
            for (size_type i = 0; i < m_data.size(); ++i)
            {
                if (m_data[i].hrank(use_capacity) > ms)
                {
                    ms = m_data[i].hrank(use_capacity);
                }
            }
            return ms;
        }
    ...
    public:
        ...
        void set_node_random(linalg::random_engine<backend> &rng)
        {
            for (size_type i = 0; i < this->nset(); ++i)
            {
                m_data[i].set_random(rng);
            }
        }
...
#endif // PYTTN_TTNS_LIB_TTN_TTN_NODES_MS_TTN_NODE_HPP_//
