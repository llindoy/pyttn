#ifndef TTNS_TREE_NODE_HPP
#define TTNS_TREE_NODE_HPP
...
public:
    ...
    node_reference at(IT first, IT last)
    {
        ...
        size_type ind = *first;
        ++first;
        if(first != last){CALL_AND_HANDLE(return (this->m_children[ind]->at(first, last)), "Failed to return node by tree index.");}
        else{return *(this->m_children[ind]);}
    }
    template <typename IT>
    ...
    size_type id_at(IT first, IT last) const
    {
        ...
        size_type ind = *first;
        ++first;
        if(first != last){CALL_AND_HANDLE(return (this->m_children[ind]->id_at(first, last)), "Failed to return node by tree index.");}
        else{return this->m_children[ind]->id();}
    }
