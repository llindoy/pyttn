...
#ifndef PYTTN_TTNS_LIB_TTN_TREE_NTREE_HPP_
#define PYTTN_TTNS_LIB_TTN_TREE_NTREE_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        node_type &at(const std::vector<size_type> &inds)
        {
            ...
            if (inds.size() == 0)
            {
                CALL_AND_RETHROW(return this->operator()())
            };
    ...
    template <typename T>
    std::ostream &operator<<(std::ostream &os, const ntree<T> &t)
    {
        os << "ntree : " << t() << ";" << std::endl;
        return os;
    }
...
#endif //  PYTTN_TTNS_LIB_TTN_TREE_NTREE_HPP_//
