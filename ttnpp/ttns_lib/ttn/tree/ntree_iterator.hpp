...
#ifndef PYTTN_TTNS_LIB_TTN_TREE_NTREE_ITERATOR_HPP_
#define PYTTN_TTNS_LIB_TTN_TREE_NTREE_ITERATOR_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        ntree_dfs_preorder_iterator(pointer node)
        {
            if (node != nullptr)
            {
    ...
    public:
        ...
        ntree_dfs_pre_post_iterator(pointer node)
        {
            if (node != nullptr)
            {
                m_stack.push(stack_elem(node, iterator(node->begin()), false));
        ...
        self_type &operator++()
        {
            ...
            while (true)
            {
                ...
                {
                    if (m_stack.top().m_times_visited == 1)
                    {
                        ++(m_stack.top().m_times_visited);
                        return *this;
                    }
    ...
    public:
        ...
        ntree_bfs_iterator(pointer node)
        {
            if (node != nullptr)
            {
...
#endif //  PYTTN_TTNS_LIB_TTN_TREE_NTREE_ITERATOR_HPP_//
