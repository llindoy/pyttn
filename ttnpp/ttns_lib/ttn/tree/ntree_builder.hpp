...
#ifndef PYTTN_TTNS_LIB_TTN_TREE_NTREE_BUILDER_HPP_
#define PYTTN_TTNS_LIB_TTN_TREE_NTREE_BUILDER_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        static tree_type balanced_tree(size_type Nleaves, size_type degree, Func &&fl, std::vector<std::vector<size_type>> &linds)
        {
            ...
            if (linds.size() != Nleaves)
            {
                linds.resize(Nleaves);
            }
        ...
        static void collapse_trivial(tree_type &tree, node_type &node, size_type index)
        {
            ...
            if (!node.is_leaf())
            {
                ...
                if (node.size() == 1 && !(node[0].is_leaf()))
                {
                    ...
                    if (curr_node->m_data < child_node->m_data)
                    {
                        child_node->m_data = curr_node->m_data;
                    }
                    ...
                    if (!is_root)
                    {
                        curr_node->m_parent->m_children[index] = child_node;
                    }
...
#endif // PYTTN_TTNS_LIB_TTN_TREE_NTREE_BUILDER_HPP_
