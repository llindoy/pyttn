...
#ifndef PYTTN_TTNS_LIB_SWEEPING_ALGORITHM_SUBSPACE_EXPANSION_SUBSPACE_EXPANSION_FULL_TWO_SITE_HPP_
#define PYTTN_TTNS_LIB_SWEEPING_ALGORITHM_SUBSPACE_EXPANSION_SUBSPACE_EXPANSION_FULL_TWO_SITE_HPP_
...
namespace ttns
{
    ...
    protected:
        ...
        linalg::singular_value_decomposition<mat_type, false> m_svd;
    ...
    public:
        ...
        {
#ifdef TRACE_LOG
            logging::trace("initialising twosite subspace expansion engine.");
#endif
            try
            {
                ...
                size_type mtwosite_capacity = 0;
                size_type mtwosite_size = 0;
                for (const auto &a : A)
                {
                    ...
                    if (!a.is_root())
                    {
                        for (size_type mode = 0; mode < nmodes; ++mode)
                        {
                            ...
                            if (c2s > mtwosite_capacity)
                            {
                                mtwosite_capacity = c2s;
                            }
                            if (s2s > mtwosite_size)
                            {
                                mtwosite_size = s2s;
                            }
                        }
        ...
        {
#ifdef TRACE_LOG
            logging::trace("clearing twosite subspace expansion engine.");
#endif
            try
            {
        ...
        bool down(hnode &A1, hnode &A2, bond_matrix_type &r, const dmat_type &pops, env_node_type &h, const env_type &op, linalg::random_engine<backend> &rng, BufType &buf, real_type svd_scale)
        {
#ifdef TRACE_LOG
            ...
            try
            {
                ...
                if (A1().shape(1) < A1().shape(0) && A1().shape(1) != 1 && subspace_spawning)
                {
                    ...
                    for (size_type i = 0; i < m_S.size(); ++i)
                    {
                        ...
                        {
                            if (std::real(m_S(i, i)) > 0)
                            {
                                sv = std::sqrt(std::real(m_S(i, i))) * svd_scale;
                            }
                            if (!std::isnan(sv))
                            {
                        ...
                        else
                        {
                            if (std::real(m_S(i, i)) > 0)
                            {
                                sv = std::real(m_S(i, i)) * svd_scale * svd_scale;
                            }
                            if (!std::isnan(sv))
                            {
                                if (sv > m_spawning_threshold)
        ...
        template <typename BufType>
        bool up(hnode &A1, hnode &A2, bond_matrix_type &r, const dmat_type &pops, env_node_type &h, const env_type &op, linalg::random_engine<backend> &rng, BufType &buf, real_type svd_scale)
        {
#ifdef TRACE_LOG
    ...
    public:
        ...
        {
#ifdef TRACE_LOG
            logging::trace("saving twosite subspace expansion engine.");
#endif
        ...
        {
#ifdef TRACE_LOG
            logging::trace("loading twosite subspace expansion engine.");
#endif
            size_type maxnmodes;
...
#endif // PYTTN_TTNS_LIB_SWEEPING_ALGORITHM_SUBSPACE_EXPANSION_SUBSPACE_EXPANSION_FULL_TWO_SITE_HPP_
