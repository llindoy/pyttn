#ifndef TTNS_SOP_TREE_NODE_HPP
#define TTNS_SOP_TREE_NODE_HPP
...
template <typename T> class sop_node_data;
...
public:
    ...
    void clear() 
    {
        try
        {
            for(size_t i = 0; i < m_term.size(); ++i){m_term[i].clear();}
            m_term.clear();
        }
