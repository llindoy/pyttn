...
template <typename T>
inline void convert_to_dense(const sOP& op, const system_modes& sysinf, const operator_dictionary<T, linalg::blas_backend>& dict, linalg::matrix<T>& ret)
{
    std::vector<size_t> dims;
    sysinf.get_mode_dimensions(dims);    
    for(size_t i = 0; i < dims.size(); ++i)
    ...
    site_operator<T> _op(op, sysinf, dict);
    CALL_AND_HANDLE(_op.todense(dims, ret), "Failed to convert sOP operator to dense matrix.");
}
...
template <typename T>
inline void convert_to_dense(const sPOP& op, const system_modes& sysinf, const operator_dictionary<T, linalg::blas_backend>& dict, linalg::matrix<T>& ret)
{
    std::vector<size_t> dims;
    sysinf.get_mode_dimensions(dims);    
    product_operator<T> _op(op, sysinf, dict);
    CALL_AND_HANDLE(_op.todense(dims, ret), "Failed to convert sOP operator to dense matrix.");
}
...
template <typename T, typename U>
inline void convert_to_dense(const sNBO<T>& op, const system_modes& sysinf, const operator_dictionary<U, linalg::blas_backend>& dict, linalg::matrix<U>& ret)
{
    std::vector<size_t> dims;
    sysinf.get_mode_dimensions(dims);    
    product_operator<U> _op(op, sysinf, dict);
    CALL_AND_HANDLE(_op.todense(dims, ret), "Failed to convert sOP operator to dense matrix.");
}
...
template <typename T, typename U>
inline void convert_to_dense(const sSOP<T>& op, const system_modes& sysinf, const operator_dictionary<U, linalg::blas_backend>& dict, linalg::matrix<U>& ret)
{
    std::vector<size_t> dims;
    sysinf.get_mode_dimensions(dims);    
    size_t nhilb = 1;
    ...
    {
        ret2.fill_zeros();
        product_operator<U> _op(t, sysinf, dict);
        CALL_AND_HANDLE(_op.todense(dims, ret2), "Failed to convert sOP operator to dense matrix.");
        ret += ret2;
    }
}
...
template <typename T, typename U>
inline void convert_to_dense(const SOP<T>& op, const system_modes& sysinf, const operator_dictionary<U, linalg::blas_backend>& dict, linalg::matrix<U>& ret)
{
    std::vector<size_t> dims;
    sysinf.get_mode_dimensions(dims);    
    size_t nhilb = 1;
    ...
    {
        ret2.fill_zeros();
        product_operator<U> _op(expand_term(t, op.operator_dictionary()), sysinf, dict);
        CALL_AND_HANDLE(_op.todense(dims, ret2), "Failed to convert sOP operator to dense matrix.");
        ret += ret2;
    }
}
