...
#ifndef PYTTN_TTNS_LIB_SOP_SSOP_HPP_
#define PYTTN_TTNS_LIB_SOP_SSOP_HPP_
...
namespace ttns
{
    ...
    template <typename T>
    std::ostream &operator<<(std::ostream &os, const sNBO<T> &op);
...
namespace ttns
{
    ...
    class sSOP;
    template <typename T>
    std::ostream &operator<<(std::ostream &os, const sSOP<T> &op);
    ...
    public:
        ...
        sSOP<T> &operator*=(const sSOP<U> &a)
        {
            ...
            terms.reserve(m_terms.size() * a.terms().size());
