...
namespace ttns
{
    ...
    public:
        ...
        size_t nmodes() const
        {
            ...
            {
                if (std::get<1>(sop) + 1 > mode)
                {
                    mode = std::get<1>(sop) + 1;
                }
            }
            return mode;
        }
    ...
    public:
        std::string hash_string() const
        {
            ...
            std::stable_sort(o.begin(), o.end(), [](const elem_type &a, const elem_type &b)
                             { return std::get<1>(a) < std::get<1>(b); });
            std::string ret;
            for (const auto &t : o)
            {
                ret += std::to_string(std::get<0>(t)) + std::string("_") + std::to_string(std::get<1>(t)) + (std::get<2>(t) ? std::string("_0") : std::string("_1"));
            }
            return ret;
        }
    ...
    public:
        ...
        void order_modes()
        {
            ...
            std::stable_sort(m_ops.begin(), m_ops.end(), [](const elem_type &a, const elem_type &b)
                             { return std::get<1>(a) < std::get<1>(b); });
            if (!m_has_hash)
            {
                get_hash();
            ...
            for (size_t i = 0; i < m_ops.size() - 1; ++i)
            {
                ...
                for (size_t j = 0; j < m_ops.size() - i - 1; ++j)
                {
                    ...
                    if (std::get<1>(m_ops[j]) > std::get<1>(m_ops[j + 1]))
                    {
                        ...
                        if (changes_parity[j] && changes_parity[j + 1])
                        {
                            flip_sign = !flip_sign;
                        }
        ...
        bool jordan_wigner(const std::vector<std::vector<std::string>> &opdict, const std::vector<bool> &is_fermion_mode) const
        {
            ...
            m_prepend_jw.resize(is_fermion_mode.size());
            std::fill(m_prepend_jw.begin(), m_prepend_jw.end(), false);
            using fermi_dict = fermion::default_fermion_operator_dictionary<double>;
...
struct std::hash<ttns::prodOP>
{
    std::size_t operator()(const ttns::prodOP &k) const
    {
        ...
        {
            return std::hash<std::string>()(k.hash_string());
        }
    }
};
...
namespace ttns
{
    ...
    public:
        inline bool set_is_fermionic_mode(std::vector<bool> &is_fermion_mode) const
        {
            ...
            std::vector<bool> mode_properties_set(is_fermion_mode.size());
            std::fill(mode_properties_set.begin(), mode_properties_set.end(), false);
            ...
            for (const auto &pop : m_terms)
            {
                for (const auto &op : std::get<0>(pop))
                {
                    ...
                    {
                        if (std::get<2>(op) != is_fermion_mode[mode])
                        {
                            return false;
                        }
                    }
    ...
    protected:
        void jordan_wigner(const std::vector<bool> &is_fermion_mode, double tol)
        {
            ...
            for (size_t i = 0; i < this->nmodes(); ++i)
            {
                ...
                {
                    contains_fermionic_operator = true;
                }
            }
    ...
    public:
        ...
        SOP &jordan_wigner(const system_modes &sys_info, double tol = 1e-15)
        {
            ...
            std::vector<bool> is_fermion_mode(nmodes);
            std::fill(is_fermion_mode.begin(), is_fermion_mode.end(), false);
...
#endif // PYTTN_TTNS_LIB_SOP_SOP_HPP_
