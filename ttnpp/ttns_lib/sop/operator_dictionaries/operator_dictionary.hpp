...
#ifndef PYTTN_TTNS_LIB_SOP_OPERATOR_DICTIONARIES_OPERATOR_DICTIONARY_HPP_
#define PYTTN_TTNS_LIB_SOP_OPERATOR_DICTIONARIES_OPERATOR_DICTIONARY_HPP_
...
namespace ttns
{
    ...
    public:
        ...
        operator_dictionary &operator=(const dict_type &o)
        {
            m_dict = o;
            return *this;
        }
        ...
        const elem_type &site_dictionary(size_t i) const
        {
            ASSERT(i < m_dict.size(), "Failed to access site dictionary index out of bounds.");
            return m_dict[i];
        }
        ...
        std::shared_ptr<op_type> query(size_t nu, const std::string &label) const
        {
            ASSERT(nu < m_dict.size(), "Failed to query element from operator dictionary.  Index out of bounds.");
            auto elem = m_dict[nu].find(label);
            if (elem == m_dict[nu].end())
            {
                return std::shared_ptr<op_type>(nullptr);
            }
            else
            {
...
#endif // PYTTN_TTNS_LIB_SOP_OPERATOR_DICTIONARIES_OPERATOR_DICTIONARY_HPP_
