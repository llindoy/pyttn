...
#ifndef PYTTN_TTNS_LIB_SOP_AUTOSOP_HPP_
#define PYTTN_TTNS_LIB_SOP_AUTOSOP_HPP_
...
namespace ttns
{
    ...
    protected:
        ...
        static inline void update_spf_union(node_type &n, size_t nterms)
        {
                 ...
                 ++prod_iter, ++bound_iter, ++index_iter)
            {
                ...
                {
                    if (prod_iter[i].m_inds.size() == bound_iter[i])
                    {
                        fully_bound = true;
                    }
                }
    ...
    protected:
        ...
        static void setup_root_node(node_type &n, size_t nterms)
        {
            ...
            utils::term_indexing_array<size_t> rempty;
            rempty.reserve(nterms);
            utils::term_indexing_array<size_t> rfull;
            utils::term_indexing_array<size_t>::complement(rempty, rfull);
    ...
    protected:
        ...
        static void optimise_spf_graph(const std::list<Bgraph> &sbpg, const std::map<std::pair<size_t, size_t>, std::vector<size_t>> &repeated_edges, const std::vector<literal::coeff<T>> &cr, node_type &n)
        {
            ...
            std::vector<auto_sop::operator_data<T>> new_spf;
            new_spf.reserve(spf.size());
            std::vector<size_t> spfr(nterms);
            n().generate_spfr(spfr, nterms);
            std::vector<size_t> mfr(nterms);
            n().generate_mfr(mfr, nterms);
                    ...
                    for (auto z : common::zip(ve, vd))
                    {
                        ...
                        if (std::find(_U.begin(), _U.end(), _u) == _U.end())
                        {
                            ...
                            if (auto it = repeated_edges.find(m_inds); it != repeated_edges.end())
                            {
                                ...
                                {
                                    accum_coeff.push_back(literal::coeff<T>(T(1.0)));
                                }
            ...
            {
                utils::term_indexing_array<size_t> r_remaining(nterms);
                utils::term_indexing_array<size_t>::complement(rbound, r_remaining);
                ...
                for (auto z : common::zip(sbpg, vertex_covers))
                {
                    ...
                    for (auto u : _U)
                    {
                        ...
                        std::vector<literal::coeff<T>> accum_coeff(1);
                        accum_coeff[0] = T(1.0);
    ...
    protected:
        ...
        static void optimise_mf_graph(const std::list<Bgraph> &sbpg, const std::map<std::pair<size_t, size_t>, std::vector<size_t>> &repeated_edges, const std::vector<literal::coeff<T>> &cr, size_t ncomposite_spf, node_type &n, bool reorder_mf = false)
        {
            ...
            std::vector<auto_sop::operator_data<T>> new_mf;
            new_mf.reserve(spf.size());
            ...
            if (reorder_mf)
            {
                ...
                std::vector<bool> ind_bound(new_mf.size());
                std::fill(ind_bound.begin(), ind_bound.end(), false);
    ...
    protected:
        ...
        static inline void update_spf_leaf_literal(const compressedSOP<T> &csop, node_type &n)
        {
            ...
            for (const auto &op : mode_ops)
            {
                ...
                {
                    utils::term_indexing_array<size_t> rinds(nterms);
                    rinds.insert(r);
    ...
    public:
        ...
        static void compressed(const SOP<T> &sop, const tree_type &A, const system_modes &sysinf, tree<auto_sop::node_op_info<T>> &bp, site_ops_type &site_ops)
        {
            ...
            tree<std::vector<size_t>> mode_tree;
            mode_tree.construct_topology(A);
...
#endif // PYTTN_TTNS_LIB_SOP_AUTOSOP_HPP_
