...
#ifndef PYTTN_UTILS_QUADRATURE_ADAPTIVE_INTEGRATE_HPP_
#define PYTTN_UTILS_QUADRATURE_ADAPTIVE_INTEGRATE_HPP_
...
namespace utils
{
    namespace quad
    {
        ...
        T adaptive_integrate_internal(F &&f, const Integ &integ, T remainder, T r, RT a, RT b, bool use_abs, RT int_tol, bool rel, RT relerror, size_t max_iterations, bool print_out, SF &&subdivision)
        {
            ...
            if (std::abs(s) == std::numeric_limits<RT>::infinity() || std::abs(t) == std::numeric_limits<RT>::infinity())
            {
                return r;
            }
            if (print_out)
            ...
            if (use_abs)
            {
                err_tol_satisfied = ((std::abs(r - (s + t)) < int_tol) ? true : err_tol_satisfied);
            }
            if (rel)
            {
                err_tol_satisfied = ((std::abs(r - (s + t)) / std::abs(remainder + s + t) < relerror) ? true : err_tol_satisfied);
            }
            if (err_tol_satisfied)
            {
                return s + t;
            }
            else
            {
                ...
                try
                {
                    return adaptive_integrate_internal(std::forward<F>(f), integ, t + remainder, s, a, c, use_abs, int_tol, rel, relerror, max_iterations - 1, print_out, std::forward<SF>(subdivision)) + adaptive_integrate_internal(std::forward<F>(f), integ, s + remainder, t, c, b, use_abs, int_tol, rel, relerror, max_iterations - 1, print_out, std::forward<SF>(subdivision));
                }
        ...
        T adaptive_integrate(F &&f, const Integ &integ, RT a, RT b, bool use_abs = true, RT int_tol = 1e-10, bool rel = false, RT relerror = 1e-12, T remainder = 0.0, size_t max_iterations = 100, bool print_out = false)
        {
            try
            {
                return adaptive_integrate_subdivision(std::forward<F>(f), integ, a, b, [](RT _a, RT _b)
                                                      { return (_a + _b) / 2.0; }, use_abs, int_tol, rel, relerror, remainder, max_iterations, print_out);
            }
            catch (const std::exception &ex)
            {
                throw;
        ...
        public:
            ...
            static result_type integrate(F &&f, lev_type &integ, const Integ &fallback, real_type x, real_type a, real_type b, real_type int_tol = 1e-14, result_type remainder = 0.0, bool rel = false, real_type relerror = 1e-12, bool print_out = false, size_t max_iterations = 500)
            {
                try
                {
                    return integrate_subdivision(std::forward<F>(f), integ, fallback, x, a, b, [](real_type _a, real_type _b)
                                                 { return (_a + _b) / 2.0; }, int_tol, remainder, rel, relerror, print_out, max_iterations);
                }
                catch (const std::exception &ex)
                {
                    throw;
            ...
            static result_type integrate_subdivision(F &&f, lev_type &integ, const Integ &fallback, real_type x, real_type a, real_type b, SF &&subdivision, real_type int_tol = 1e-14, result_type remainder = 0.0, bool rel = false, real_type relerror = 1e-12, bool print_out = false, size_t max_iterations = 500)
            {
                ...
                auto fcyl = [x, &f, &integ](real_type _r)
                { return f(_r) * integ.oscillatory_function(x * _r); };
                ...
                if (std::abs(r - (s + t)) / scale < int_tol)
                {
                    return s + t;
                }
                else
                {
                    ...
                    try
                    {
                        return integrate_internal(std::forward<F>(f), integ, fallback, remainder + t, s, x, a, c, int_tol, rel, relerror, print_out, max_iterations - 1, std::forward<SF>(subdivision)) + integrate_internal(std::forward<F>(f), integ, fallback, remainder + s, t, x, c, b, int_tol, rel, relerror, print_out, max_iterations - 1, std::forward<SF>(subdivision));
                    }
                    catch (const std::exception &ex)
                    {
                        throw;
        ...
        public:
            ...
            fresult_type fourier(F &&f, real_type x, real_type a, real_type b, real_type iguess = 1e3, real_type int_tol = 1e-14, fresult_type remainder = 0.0, bool rel = false, real_type relerror = 1e-12, bool print_out = false, size_t max_iterations = 500)
            {
                if (b == std::numeric_limits<real_type>::infinity())
                {
                    if (std::abs(x) < 1e-5)
                    {
                        CALL_AND_RETHROW(return upper_real_integral([&](const real_type &w)
                                                                    { return f(w) * fresult_type(std::cos(x * w), std::sin(x * w)); }, a, iguess, int_tol, remainder, rel, relerror, print_out, max_iterations));
                    }
                    else
                    {
                ...
                else
                {
                    if (std::abs(x) / (b - a) < 10)
                    {
                        CALL_AND_RETHROW(return definite_integral([&](const real_type &w)
                                                                  { return f(w) * fresult_type(std::cos(x * w), std::sin(x * w)); }, a, b, int_tol, remainder, rel, relerror, print_out, max_iterations));
                    }
                    else
                    {
            ...
            result_type cosine(F &&f, real_type x, real_type a, real_type b, real_type iguess = 1e3, real_type int_tol = 1e-14, result_type remainder = 0.0, bool rel = false, real_type relerror = 1e-12, bool print_out = false, size_t max_iterations = 500)
            {
                if (b == std::numeric_limits<real_type>::infinity())
                {
                    ...
                    {
                        CALL_AND_RETHROW(return upper_real_integral([&](const real_type &w)
                                                                    { return f(w) * std::cos(x * w); }, a, iguess, int_tol, remainder, rel, relerror, print_out, max_iterations));
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        CALL_AND_RETHROW(return definite_integral([&](const real_type &w)
                                                                  { return f(w) * std::cos(x * w); }, a, b, int_tol, remainder, rel, relerror, print_out, max_iterations));
                    }
                    else
                    {
            ...
            result_type sine(F &&f, real_type x, real_type a, real_type b, real_type iguess = 1e3, real_type int_tol = 1e-14, result_type remainder = 0.0, bool rel = false, real_type relerror = 1e-12, bool print_out = false, size_t max_iterations = 500)
            {
                if (b == std::numeric_limits<real_type>::infinity())
                {
                    ...
                    {
                        CALL_AND_RETHROW(return upper_real_integral([&](const real_type &w)
                                                                    { return f(w) * std::sin(x * w); }, a, iguess, int_tol, remainder, rel, relerror, print_out, max_iterations));
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        CALL_AND_RETHROW(return definite_integral([&](const real_type &w)
                                                                  { return f(w) * std::sin(x * w); }, a, b, int_tol, remainder, rel, relerror, print_out, max_iterations));
                    }
                    else
                    {
            ...
            U upper_real_integral(F &&f, real_type a, real_type b, real_type int_tol = 1e-14, U remainder = 0.0, bool rel = false, real_type relerror = 1e-12, bool print_out = false, size_t max_iterations = 500)
            {
                try
                {
                    ...
                    while (continue_iter)
                    {
                        ...
                        if (std::abs(local) < relerror * std::abs(res) || std::abs(local) < 1e-15)
                        {
                            continue_iter = false;
                        }
                        res += local;
...
#endif // PYTTN_UTILS_QUADRATURE_ADAPTIVE_INTEGRATE_HPP_//
