...
#ifndef PYTTN_UTILS_QUADRATURE_GAUSSIAN_QUADRATURE_GAUSS_LAGUERRE_QUADRATURE_HPP_
#define PYTTN_UTILS_QUADRATURE_GAUSSIAN_QUADRATURE_GAUSS_LAGUERRE_QUADRATURE_HPP_
...
namespace utils
{
    namespace quad
    {
        namespace gauss
        {
            ...
            public:
                template <typename F>
                static auto impl(F &&f, const RT &exponent, const RT &alpha,
            ...
            public:
                associated_laguerre(size_t N, const RT &alpha, bool normalise_weights = true)
                    : m_alpha(alpha), m_normalise_weights(normalise_weights), m_w(N), m_x(N)
                {
                ...
                void construct_quadrature_rule()
                {
                    ...
                    for (size_t i = 0; i < m_w.size(); ++i)
                    {
                        vals(i) = vecs(0, i) * vecs(0, i) *
                                  (m_normalise_weights ? std::tgamma(m_alpha + 1) : 1.0);
                    }
                    m_w = vals;
                }
                                ...
                                const RT &exponent = 1.0) const -> decltype(f(exponent))
                {
                    ...
                    return laguerre_quad<T, backend>::impl(std::forward<F>(f), exponent,
                                                           m_alpha, m_w, m_x) *
                           (m_normalise_weights ? 1.0 : std::tgamma(m_alpha + 1));
                }

                RT weight_function(RT x, RT exponent = 1.0)
                {
                    return std::pow(x, m_alpha) * std::exp(-x * exponent);
                }
...
#endif // PYTTN_UTILS_QUADRATURE_GAUSSIAN_QUADRATURE_GAUSS_LAGUERRE_QUADRATURE_HPP_
