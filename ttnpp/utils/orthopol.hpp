...
#ifndef PYTTN_UTILS_ORTHOPOL_HPP_
#define PYTTN_UTILS_ORTHOPOL_HPP_
...
namespace utils
{
    ...
    void polynomial_moments(std::vector<T> &modified_moments, const orthopol<T> &orthref, size_t nmax, F &&f, T rel_tol = 1e-14, T tol = 1e-15, T scale_factor = 1, size_t quadrature_order = 100, size_t max_order = 100, T minbound = 0, T maxbound = std::numeric_limits<T>::max())
    {
        try
        {
            ...
            {
                modified_moments[i] = quad::adaptive_integrate([&](T x)
                                                               { return orthref.monic(x, i) * f(x) * scale_factor; }, gauss_leg, xmin, xmax, false, tol, true, rel_tol, 0.0, max_order, false);
    ...
    void nonclassical_polynomial(orthopol<T> &orth, const orthopol<T> &orthref, const linalg::vector<T> &modified_moments, T scale_factor = 1)
    {
        ...
        for (size_t i = 0; i < 2 * nmax; ++i)
        {
            sigma(1, i) = modified_moments[i];
        }

        // need to modify the alpha and beta parameters given the shift
        linalg::vector<T> aref(nmax * 2);
        linalg::vector<T> bref(nmax * 2);
        for (size_t i = 0; i < 2 * nmax; ++i)
        {
            aref(i) = orthref.alpha(i);
        }
        bref(0) = 0;
        for (size_t i = 1; i < 2 * nmax; ++i)
        {
            bref(i) = orthref.beta(i - 1);
            bref(i) *= bref(i);
        }

        // set the j elements
        linalg::vector<T> a(nmax);
        linalg::vector<T> b(nmax);
    ...
    void nonclassical_polynomial(orthopol<T> &orth, const vec_type &x, const vec_type &w)
    {
        ...
        linalg::vector<T> alpha(N);
        alpha.fill_zeros();
        alpha(0) = 1.0;
        linalg::vector<T> beta(w.size());
        beta.fill_zeros();
...
#endif // PYTTN_UTILS_ORTHOPOL_HPP_//
