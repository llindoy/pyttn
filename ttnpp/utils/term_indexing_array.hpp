...
#ifndef PYTTN_UTILS_TERM_INDEXING_ARRAY_HPP_
#define PYTTN_UTILS_TERM_INDEXING_ARRAY_HPP_
...
namespace utils
{
    ...
    public:
        ...
        public:
            ...
            indexing_iterator(setiter it, setiter itend, bool complement, size_t maxr) : m_it(it), m_itend(itend), m_complement(complement), m_maxr(maxr)
            {
                get_initial_value();
            }
            indexing_iterator(setiter it, setiter itend, bool complement, size_t maxr, size_t fval) : m_it(it), m_itend(itend), m_complement(complement), m_maxr(maxr), m_curr_value(fval) {}
    ...
    public:
        ...
        term_indexing_array(const std::vector<size_type> &r, size_type maxr)
        ...
        bool operator==(const term_indexing_array &o) const
        {
            ...
            {
                if (o.size() != this->size() || m_maxr != o.m_maxr)
                {
                    return false;
                }
    ...
    public:
        bool contains(size_type v) const
        {
            ...
            if (!m_store_complement)
            {
                return m_r.count(v) == 1;
            }
            else
            {
                return m_r.count(v) == 0;
            }
        }
    ...
    protected:
        void test_inversion()
        {
            if (m_r.size() >= m_maxr / 2)
            {
                ...
                if (m_r.size() == m_maxr / 2 && !m_store_complement)
                {
                    return;
                }
...
#endif // PYTTN_UTILS_TERM_INDEXING_ARRAY_HPP_
