...
#ifndef PYTTN_UTILS_OCCUPATION_NUMBER_BASIS_INDEXING_HPP_
#define PYTTN_UTILS_OCCUPATION_NUMBER_BASIS_INDEXING_HPP_
    ...
    public:
        ...
        void initialise(size_t L, size_t K)
        {
            try
            {
                ...
                bt::m_localdims.resize(K);
                std::fill(bt::m_localdims.begin(), bt::m_localdims.end(), L);
        ...
        virtual size_t get_index(const std::vector<size_t> &ind) const
        {
            ...
            if (itr == m_nk.end())
            {
                RAISE_EXCEPTION("Occupation number sequence not found.");
            }
            else if (*itr != ind)
            {
                RAISE_EXCEPTION("Occupation number sequence not found.");
            ...
            {
                return std::distance(m_nk.begin(), itr);
            }
        }
        ...
        virtual bool contains_adjacent_state(size_t i, size_t k1, size_t k2) const
        {
            ...
            if (!contains_lowered_state(i, k1))
            {
                return false;
            }
            ...
            for (size_t i = 0; i < bt::m_D; ++i)
            {
                ...
                for (size_t j = 0; j < bt::m_K; ++j)
                {
                    ...
                    else
                    {
                        ...
                        {
                            m_nkm1[i][j] = std::distance(m_nk.begin(), itr);
                            ++bt::m_nm1;
                        }
                    }
                    ...
                    {
                        m_nkp1[i][j] = std::distance(m_nk.begin(), itr);
                        ++bt::m_np1;
                    }
    ...
    public:
        ...
        virtual bool contains_state(const std::vector<size_t> &ind) const
        {
            ...
            for (size_t k = 0; k < m_bases.size(); ++k)
            {
                ...
                if (!m_bases[k]->contains_state(rloc))
                {
                    return false;
                }
            }
...
#endif // PYTTN_UTILS_OCCUPATION_NUMBER_BASIS_INDEXING_HPP_//
