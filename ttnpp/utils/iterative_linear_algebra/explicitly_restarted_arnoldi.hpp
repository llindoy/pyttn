...
#ifndef PYTTN_UTILS_ITERATIVE_LINEAR_ALGEBRA_EXPLICITLY_RESTARTED_ARNOLDI_HPP_
#define PYTTN_UTILS_ITERATIVE_LINEAR_ALGEBRA_EXPLICITLY_RESTARTED_ARNOLDI_HPP_
...
namespace utils
{
    ...
    public:
        explicitly_restarted_arnoldi()
            : m_arnoldi(), m_krylov_dim(0), m_istride(1), m_max_iter(1),
              m_eps(std::numeric_limits<real_type>::epsilon() * 1e3) {}
        explicitly_restarted_arnoldi(
            size_type krylov_dim, size_type dim,
            real_type eps = std::numeric_limits<real_type>::epsilon() * 1e3)
            : m_arnoldi(), m_krylov_dim(krylov_dim), m_istride(1), m_max_iter(1),
              m_eps(eps)
        {
        ...
        explicitly_restarted_arnoldi &
        operator=(const explicitly_restarted_arnoldi &o) = default;
        explicitly_restarted_arnoldi &
        operator=(explicitly_restarted_arnoldi &&o) = default;
        ...
        void resize(size_type krylov_dim, size_type dim)
        {
            try
            {
                ...
                CALL_AND_HANDLE(m_rvecs.resize(krylov_dim, krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_rvecsd.resize(1, krylov_dim),
                                "Failed to resize the temp1 vector.");
        ...
        operator()(vec_type &x, value_type &E, Args &&...args)
        {
            try
            {
                ...
                ASSERT(neigs <= m_krylov_dim, "Cannot compute more eigenvalues than the "
                ...
                E = value_type(0);
                CALL_AND_HANDLE(m_residues.resize(m_max_iter),
                                "Failed to resize residues array.");
                ...
                for (size_t iter = 0; iter <= m_max_iter; ++iter)
                {
                    ...
                    if (iter != m_max_iter)
                    {
                        for (iend = iend_start; iend < krylov_dim + m_istride && keep_running;
                             iend += m_istride)
                        {
                            if (iend > krylov_dim)
                            {
                            ...
                            try
                            {
                                ...
                                CALL_AND_HANDLE(ended_early = m_arnoldi.partial_krylov_step(
                                                    x, scalefactor, istart, iend,
                                                    std::forward<Args>(args)...),
                                                "Failed to construct the krylov subspace using a "
                                                "arnoldi iteration");
                                if (ended_early)
                                {
                                    keep_running = false;
                                ...
                                // sort the eigenvalues based on norm and take the one with the
                                // largest as our value
                                {
    ...
    protected:
        void get_vecs_and_vals(value_type &E,
                               linalg::vector<value_type, backend_type> &x)
        {
            try
            {
...
#endif // PYTTN_UTILS_ITERATIVE_LINEAR_ALGEBRA_EXPLICITLY_RESTARTED_ARNOLDI_HPP_
