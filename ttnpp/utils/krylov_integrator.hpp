...
#ifndef PYTTN_UTILS_KRYLOV_INTEGRATOR_HPP_
#define PYTTN_UTILS_KRYLOV_INTEGRATOR_HPP_
...
namespace utils
{
    ...
    protected:
        ...
        // we construct the eigenvalues of the upper hessenberg matrix obtained from
        // arnoldi on the gpu
        linalg::eigensolver<
            linalg::upper_hessenberg_matrix<value_type, linalg::blas_backend>>
            m_eigensolver;
    ...
    public:
        krylov_integrator()
            : m_arnoldi(), m_krylov_dim(0), m_cur_order(0), m_istride(1),
              m_eps(std::numeric_limits<real_type>::epsilon() * 1e3), m_gamma(0.8),
              m_delta(0.9) {}
        krylov_integrator(size_type krylov_dim, size_type dim,
                          real_type eps = std::numeric_limits<real_type>::epsilon() *
                                          1e3)
            : m_arnoldi(), m_krylov_dim(krylov_dim), m_cur_order(0), m_istride(1),
              m_eps(eps), m_gamma(0.8), m_delta(0.9)
        {
            CALL_AND_HANDLE(resize(krylov_dim, dim),
                            "Failed to construct krylov subspace integrator.");
        }
        ...
        void resize(size_type krylov_dim, size_type dim)
        {
            try
            {
                ...
                CALL_AND_HANDLE(m_arnoldi.resize(krylov_dim, dim),
                                "Failed to resize arnoldi iteration engine.");
                CALL_AND_HANDLE(m_eigensolver.resize(krylov_dim, false),
                                "Failed to resize upper_hessenberg_eigensolver.");
                CALL_AND_HANDLE(m_e1.resize(krylov_dim),
                                "Failed to resize the e1 vector.");

                CALL_AND_HANDLE(m_vals.resize(krylov_dim, krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_expmat.resize(krylov_dim, krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_sorted_vals.resize(krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_sorted_vals2.resize(krylov_dim),
                                "Failed to resize the temp1 vector.");

                CALL_AND_HANDLE(m_rvecs.resize(krylov_dim, krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_rvecsd.resize(krylov_dim, krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_lvecs.resize(krylov_dim, krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_temp1.resize(krylov_dim),
                                "Failed to resize the temp1 vector.");
                CALL_AND_HANDLE(m_temp2.resize(krylov_dim),
                                "Failed to resize the temp2 vector.");
                CALL_AND_HANDLE(m_tempr.resize(dim),
                                "Failed to resize the temp2 vector.");
                m_e1.fill_zeros();
                m_e1(0) = 1;
            }
        ...
        void clear()
        {
            ...
            {
                m_krylov_dim = 0;
                CALL_AND_HANDLE(m_arnoldi.clear(),
                                "Failed to resize arnoldi iteration engine.");
                CALL_AND_HANDLE(m_eigensolver.clear(),
                                "Failed to resize upper_hessenberg_eigensolver.");
        ...
        typename std::enable_if<
            linalg::is_same_backend<vec_type,
                                    linalg::vector<value_type, backend_type>>::value,
            size_type>::type
        operator()(vec_type &x, real_type dt, value_type coeff, Args &&...args)
        {
            try
            {
                ...
                ASSERT(m_istride <= krylov_dim,
                       "The stride for determining error estimates is too large.");

                // construct the krylov subspace and store the final matrix element
                // required for computing error estimates and matrix exponentials
                CALL_AND_HANDLE(m_tempr.resize(x.size()),
                                "Failed to resize the working buffer.");
                CALL_AND_HANDLE(m_arnoldi.resize(krylov_dim, x.size()),
                                "Failed to resize krylov subspace.");
                ...
                while (dt_completed / dt < 1)
                {
                    ...
                    real_type err_res;
                    determine_order(x, dt_trial, coeff, scale_factor, nan_encountered,
                                    err_res, std::forward<Args>(args)...);
                    nevals += m_cur_order;
                    ...
                    if (!nan_encountered)
                    {
                        ...
                        {
                            dt_trial_next =
                                m_gamma * std::pow(m_eps / err, real_type(1.0 / m_cur_order)) *
                                (dt_trial);
                            ...
                            CALL_AND_HANDLE(err_res = local_error_estimate(
                                                std::abs(dt_trial), coeff / scale_factor),
                                            "Failed to compute local error estimate.");
                            err = err_res / std::abs(dt_trial);
                            error_tol_satisfied = err < m_delta * m_eps;
                        }

                        CALL_AND_HANDLE(integrate_step(x, dt_trial, coeff / scale_factor),
                                        "Failed to perform the integration step.");
                        dt_completed += dt_trial;

                        dt_trial_next =
                            m_gamma *
                            std::pow(m_eps / err_res, real_type(1.0 / m_cur_order)) *
                            (dt_trial);
                        if (std::abs(dt_trial_next) < 0.1 * std::abs(dt_trial))
                        {
                            dt_trial_next = 0.1 * dt_trial;
                    ...
                    else
                    {
                        ...
                        {
                            RAISE_NUMERIC("Nan encountered at first element of krylov subspace "
                                          "iteration.");
                        }
                        scale_factor /= 10;
                    }
        ...
        typename std::enable_if<
            linalg::is_same_backend<vec_type,
                                    linalg::vector<value_type, backend_type>>::value,
            size_type>::type
        eigs(vec_type &x, linalg::diagonal_matrix<value_type, backend_type> &vals,
             linalg::matrix<value_type, backend_type> &vecs, real_type scalefactor,
             size_type neigs, size_type maxdim, real_type errtol, Args &&...args)
        {
            try
            {
                size_type krylov_dim = m_krylov_dim < x.size() ? m_krylov_dim : x.size();
                if (maxdim > krylov_dim)
                {
                    maxdim = krylov_dim;
                }
                ASSERT(m_istride <= krylov_dim,
                ...
                ASSERT(neigs < m_krylov_dim,
                       "Cannot compute more eigenvalues than dimension of object.");
                if (neigs > x.size())
                {
                    neigs = x.size();
                }
                ...
                CALL_AND_HANDLE(m_arnoldi.resize(krylov_dim, x.size()),
                                "Failed to resize krylov subspace.");
                CALL_AND_HANDLE(m_arnoldi.reset_zeros(),
                                "Failed to reset arnoldi iteration.");
                CALL_AND_HANDLE(m_sorted_vals.resize(krylov_dim),
                                "Failed to resize the working buffer.");
                CALL_AND_HANDLE(m_sorted_vals2.resize(neigs),
                                "Failed to resize the eigenvalue storage array.");

                // compute the arnoldi iteration.

                // now compute the eigenvalues in the arnoldi subspace
                CALL_AND_HANDLE(m_vals.resize(krylov_dim),
                                "Failed to resize the working buffer.");
                CALL_AND_HANDLE(vecs.resize(neigs, x.size()),
                                "Failed to resize the working buffer.");
                ...
                for (iend = order_start; iend <= krylov_dim + m_istride && keep_running;
                     iend += m_istride)
                {
                    if (iend > maxdim)
                    {
                    ...
                    try
                    {
                        ...
                        CALL_AND_HANDLE(
                            ended_early = m_arnoldi.partial_krylov_step(
                                x, scalefactor, istart, iend, std::forward<Args>(args)...),
                            "Failed to construct the krylov subspace using a arnoldi "
                            "iteration");
                        auto H = m_arnoldi.H();
                        ...
                        // sort the eigenvalues based on norm
                        {
                            ...
                            std::sort(m_sorted_vals.begin(), m_sorted_vals.end(),
                                      [](const std::pair<value_type, size_type> &a,
                                         const std::pair<value_type, size_type> &b)
                                      {
                        ...
                        if (first_call)
                        {
                            CALL_AND_HANDLE(m_sorted_vals2 = m_sorted_vals,
                                            "Failed to copy eigenvalue array.");
                            first_call = false;
                        ...
                        else
                        {
                            ...
                            {
                                if (abs((std::get<0>(m_sorted_vals[i]) -
                                         std::get<0>(m_sorted_vals2[i])) /
                                        std::get<0>(m_sorted_vals[i])) > errtol)
                                {
                                    err_tol_satisfied = false;
                                }
                        ...
                        if (err_tol_satisfied || ended_early)
                        {
                            keep_running = false;
                        }
                    }
                ...
                CALL_AND_HANDLE(sort_vecs_and_vals(vals, vecs, scalefactor),
                                "Failed to sort the eigenvalues.");
                return iend;
            }
            catch (const common::invalid_value &ex)
            {
    ...
    protected:
        void
        sort_vecs_and_vals(linalg::diagonal_matrix<value_type, backend_type> &vals,
                           linalg::matrix<value_type, backend_type> &vecs,
                           real_type scalefactor)
        {
            try
            {
                // sort and scale eigenvalues
                CALL_AND_HANDLE(m_vals.resize(m_sorted_vals2.size()),
                                "Failed to resize vals array.");
                for (size_type i = 0; i < m_sorted_vals2.size(); ++i)
                {
                    m_vals[i] = std::get<0>(m_sorted_vals2[i]);
                }
                CALL_AND_HANDLE(vals = m_vals / scalefactor,
                                "Failed to copy the eigenvalues.");

                // sort eigenvectors in krylov subspace
                CALL_AND_HANDLE(m_lvecs = linalg::adjoint(m_rvecs),
                                "Failed to store rvecs in temporary.");
                for (size_type i = 0; i < m_sorted_vals2.size(); ++i)
                {
                ...
                CALL_AND_HANDLE(vecs = m_rvecsd * m_arnoldi.Q(),
    ...
    protected:
        template <typename vec_type, typename... Args>
        void determine_order(vec_type &x, real_type dt, value_type coeff,
                             real_type scale_factor, bool &nan_encountered,
                             real_type &err_res, Args &&...args)
        {
            ...
            size_type counter = 0;
            bool keep_running = true;
            for (iend = order_start; iend < m_krylov_dim + m_istride;
                 iend += m_istride)
            {
                if (iend > krylov_dim)
                {
                ...
                try
                {
                    ...
                    CALL_AND_HANDLE(
                        ended_early = m_arnoldi.partial_krylov_step(
                            x, scale_factor, istart, iend, std::forward<Args>(args)...),
                        "Failed to construct the krylov subspace using a arnoldi "
                        "iteration");
                    if (ended_early)
                    {
                    ...
                    mdtres[counter] = std::make_pair(
                        std::abs(std::pow(m_eps / (err_res / std::abs(dt)),
                                          real_type(1.0 / m_arnoldi.current_krylov_dim())) *
                                 (std::abs(dt))),
                        m_arnoldi.current_krylov_dim());
                    ++counter;
                }
                catch (const std::exception &ex)
                {
                ...
                if (!nan_encountered)
                {
                    ...
                    // if the current error is much larger than the previous smallest error
                    // then we stop attempting to adapt the order at this stage and attemp
                    ...
                    if (emin > 0 && err_res > emin * 1e5)
                    {
                        ...
                        auto H2 = m_arnoldi.H();
                        CALL_AND_HANDLE(m_eigensolver(H2, m_vals, m_rvecs, m_lvecs, true),
                                        "Failed to diagonalise upper hessenberg matrix.");
                        CALL_AND_HANDLE(err_res = local_error_estimate(std::abs(dt),
                                                                       coeff / scale_factor),
                                        "Failed to compute local error estimate.");
                        return;
                    }
        ...
        void integrate_step(vec_type &x, real_type dt, value_type coeff)
        {
            ...
            {
                RAISE_EXCEPTION("Failed to compute krylov subspace representation of the "
                                "propagator.");
            }
            try
            {
        ...
        real_type local_error_estimate(real_type dt, value_type coeff)
        {
            try
            {
                ...
                for (size_type k = 0; k < m_vals.shape(0); ++k)
                {
                    ...
                    accum +=
                        m_rvecs(m_vals.size(0) - 1, k) * conj(m_lvecs(0, k)) * phi1(rekc);
                }
                ...
                {
                    RAISE_NUMERIC(
                        "Nan encountered in calcaulation of local error estimate.");
                }
                return ret;
            }
            catch (const common::invalid_value &ex)
...
#endif // PYTTN_UTILS_KRYLOV_INTEGRATOR_HPP_
