...
#ifndef PYTTN_UTILS_BATH_AAA_HPP_
#define PYTTN_UTILS_BATH_AAA_HPP_
...
namespace utils
{
    ...
    public:
        ...
        baryocentric_rational_function(const linalg::vector<complex_type> &z,
                                       const linalg::vector<complex_type> &f,
                                       const linalg::vector<complex_type> &w)
            : m_z(z), m_f(f), m_w(w)
        {
            ASSERT(
                m_z.size() == m_f.size() && m_z.size() == m_w.size(),
                "Failed to construct baryocentric rational function.  Invalid inputs.");
        }
        ~baryocentric_rational_function() {}

        baryocentric_rational_function &
        operator=(const baryocentric_rational_function &o) = default;
        baryocentric_rational_function &
        operator=(baryocentric_rational_function &&o) = default;
        ...
        // function for computing the poles, residues and zeros of the baryocentric
        // rational function
        void prz(linalg::vector<complex_type> &p, linalg::vector<complex_type> &r,
                 linalg::vector<complex_type> &z, T tol = T(1e-6),
                 size_t qorder = 24) const
        {
            size_t m = m_w.size();
    ...
    private:
        void compute_residues(const linalg::vector<complex_type> &p,
                              linalg::vector<complex_type> &r, T tol,
                              size_t qorder) const
        {
            ...
            for (size_t i = 0; i < p.size(); ++i)
            {
                ...
                // now divide the distance by 10 so that we can evaluate the pole using
                // cauchy's theorem without overlapping with other poles
    ...
    public:
        ...
        baryocentric_rational_function<T>
        operator()(Func &&func, const linalg::vector<U> &_Z, Args &&...args)
        {
            size_t M = _Z.size();
            ...
            CALL_AND_HANDLE(return compute(_FZ, _Z),
                                   "Failed to compute AAA decomposition");
        }

        template <typename U2>
        baryocentric_rational_function<T>
        compute(const linalg::vector<complex_type> &_FZ,
                const linalg::vector<U2> &_Z)
        {
            try
            {
                ASSERT(_Z.size() == _FZ.size(),
                       "Input vectors are not of the same size.");
...
#endif // PYTTN_UTILS_BATH_AAA_HPP_//
