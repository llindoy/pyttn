...
namespace linalg
{
    ...
    public:
        ...
        template <typename T>
        struct eCop<complex<T>>
        {
            inline complex<T> operator()(const complex<T> &a) { return conj(a); }
        };
        template <typename T>
        struct eNop
        {
...
#pragma omp parallel for schedule(static)
    ...
    public:
        ...
        template <typename T>
        static inline typename std::enable_if<is_valid_value_type<complex<T>>::value, void>::type complex_conjugate(blas_int_type size, const complex<T> *const X, complex<T> *const Y)
        {
        ...
        static inline void assign_real_to_complex(const T *src, size_type n, T *dest, T beta = T(0))
        {
            ...
            else
            {
                for (size_type i = 0; i < n; ++i)
                {
                    dest[i] = beta * src[i] + complex<T>(src[i], 0.0);
                }
            }
        }
    ...
    public:
        ...
        static inline void csrmv(transform_type opA, bool conjB, blas_int_type m, blas_int_type /*n*/, size_type /*nnz*/, complex<T> alpha, const complex<T> *A, const int *rowptr, const int *colind, const complex<T> *X, blas_int_type incx, complex<T> beta, complex<T> *Y, blas_int_type incy)
        {
            ...
            if (opA == op_c)
            {
                if (conjB)
                {
                    eblas_kernels::csrmv::eval(m, alpha, A, rowptr, colind, X, incx, beta, Y, incy, [](const complex<T> &a)
                                               { return conj(a); }, [](const complex<T> &a)
                                               { return conj(a); });
                }
                else
                {
    ...
    public:
        ...
        {
            csrmm_kernel_selector<4, 4>(opres, opA, opB, m, n, alpha, A, rowptr, colind, B, ldb, beta, C, ldc, [](const complex<T> &a)
                                        { return conj(a); }, [](const complex<T> &a)
                                        { return a; });
        }
