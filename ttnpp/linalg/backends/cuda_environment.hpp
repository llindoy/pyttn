...
namespace linalg
{
    ...
    public:
        ...
        {
            m_device_id = std::move(other.m_device_id);
            other.m_device_id = 0;
            m_prop = std::move(other.m_prop);
            m_initialised = std::move(other.m_initialised);
            other.m_initialised = false;
            m_nstreams = std::move(other.m_nstreams);
            other.m_nstreams = 0;
            m_current_stream = std::move(other.m_current_stream);
            other.m_current_stream = 0;
            m_streams = std::move(other.m_streams);
            other.m_streams.clear();
            m_cublas_handle = std::move(other.m_cublas_handle);
            other.m_cublas_handle = 0;
            m_cusolver_dn_handle = std::move(other.m_cusolver_dn_handle);
            other.m_cusolver_dn_handle = 0;
            m_cusparse_handle = std::move(other.m_cusparse_handle);
            other.m_cusparse_handle = 0;
            m_cutensor_handle = std::move(other.m_cutensor_handle);
            other.m_cutensor_handle = 0;
            return *this;
        }
        ...
        void init(int device_id, int nstreams = 1)
        {
            ...
            for (size_type i = 0; i < m_nstreams - 1; ++i)
            {
        ...
        void destroy()
        {
            if (m_initialised)
            {
                ...
                for (size_type i = 0; i < m_nstreams - 1; ++i)
                {
                    CALL_AND_HANDLE(cuda_safe_call(cudaStreamDestroy(m_streams[i])), "Failed to destroy cuda_environment object.  Error when destroying stream objects.");
                }
        ...
        {
            ++m_current_stream;
            m_current_stream = (m_current_stream == m_nstreams) ? 0 : m_current_stream;
        }
        ...
        const cutensorHandle_t &cutensor_handle() const
        {
    ...
    inline std::ostream &operator<<(std::ostream &out, const cuda_environment &inst)
    {
