...
#ifndef PYTTN_LINALG_BACKENDS_CUBLAS_WRAPPER_HPP_
#define PYTTN_LINALG_BACKENDS_CUBLAS_WRAPPER_HPP_
...
static inline void cublas_safe_call(cublasStatus_t err)
{
    if (err != CUBLAS_STATUS_SUCCESS)
    {
        RAISE_EXCEPTION_STR(cublasGetErrorName(err));
    }
}
...
namespace linalg
{
    namespace cublas
    {
        ...
        static inline complex<float> dot(cublasHandle_t handle, bool conj, int N, const complex<float> *X, int INCX, const complex<float> *Y, int INCY)
        {
            ...
            {
                CALL_AND_RETHROW(cublas_safe_call(cublasCdotu(handle, N, (const cuComplex *)X, INCX, (const cuComplex *)Y, INCY, (cuComplex *)&result)));
            }
            return result;
        }
        static inline complex<double> dot(cublasHandle_t handle, bool conj, int N, const complex<double> *X, int INCX, const complex<double> *Y, int INCY)
        {
            ...
            {
                CALL_AND_RETHROW(cublas_safe_call(cublasZdotu(handle, N, (const cuDoubleComplex *)X, INCX, (const cuDoubleComplex *)Y, INCY, (cuDoubleComplex *)&result)));
            }
            return result;
        }
...
#endif // PYTTN_LINALG_BACKENDS_CUBLAS_WRAPPER_HPP_//
