...
#ifndef PYTTN_LINALG_BACKENDS_EXTENDED_BLAS_KERNELS_HPP_
#define PYTTN_LINALG_BACKENDS_EXTENDED_BLAS_KERNELS_HPP_
...
namespace linalg
{
    namespace eblas_kernels
    {
        ...
        {
            template <typename T, typename OPA>
            static inline void eval(int /* n */, const T & /* alpha */, T * /* A */, const T & /* beta */, OPA && /* opa */) { RAISE_EXCEPTION("Should never get here"); }
        };
        ...
        {
            template <typename T, typename OPA>
            static inline void eval(int m, int n, const T &alpha, const T * A, const T &beta, T * B, OPA &&opa)
            {
                if (beta == T(0.0))
                {
                    // #ifdef USE_OPENMP
            ...
            template <typename T, typename OPA>
            static inline void eval(int m, int n, int batchCount, const T &alpha, const T * A, const T &beta, T * B, OPA &&opa)
            {
                        ...
                        for (int i = 0; i < n; i += blockSizeN)
                        {
                            for (int j = 0; j < m; j += blockSizeM)
                            {
                                ...
                                for (int k = i; k < i + blockSizeN && k < n; ++k)
                                {
                                    for (int l = j; l < j + blockSizeM && l < m; ++l)
                                    {
                                        B[bskip + k + l * n] = alpha * opa(A[bskip + l + k * m]);
                                    }
                                }
                            }
                        }
                        ...
                        for (int i = 0; i < n; i += blockSizeN)
                        {
                            for (int j = 0; j < m; j += blockSizeM)
                            {
                                ...
                                for (int k = i; k < i + blockSizeN && k < n; ++k)
                                {
                                    for (int l = j; l < j + blockSizeM && l < m; ++l)
                                    {
                                        B[bskip + k + l * n] = alpha * opa(A[bskip + l + k * m]) + B[bskip + k + l * n] * beta;
                                    }
                                }
                            }
                        }
        ...
        {
            template <typename T, typename OPA>
            static inline void eval(int /* m */, int /* n */, const T & /* alpha */, const T * /* A */, const T & /* beta */, T * /* B */, OPA && /* opa */) { RAISE_EXCEPTION("Should never get here"); }
            template <typename T, typename OPA>
            static inline void eval(int /* m */, int /* n */, int /* batchCount */, const T & /* alpha */, const T * /* A */, const T & /* beta */, T * /* B */, OPA && /* opa */) { RAISE_EXCEPTION("Should never get here"); }
        };
        ...
        protected:
            ...
            static inline void inner_loop(const size_t m, const size_t n, const size_t k, T1 alpha, const T2 *A, const size_t inca, const T3 *B, const size_t ldb, T4 beta, T5 *C, const size_t ldc, OPA &&opa, OPB &&opb, FuncA &&fa, FuncCalc1 &&fcalc1, FuncCalc2 &&fcalc2)
            {
                ...
                for (size_t j = 0; j < min_km; ++j)
                {
                    ...
                    {
                        C[cind + i] = fcalc1(C, opaj, B, beta, i, cind, bind, std::forward<OPB>(opb));
                    }
                }
                // #ifdef USE_OPENMP
                //         #pragma omp parallel for schedule(static)
                // #endif
                for (size_t j = min_km; j < m; ++j)
                {
                    size_t cind = ldc * j;
                    for (size_t i = 0; i < n; ++i)
                    {
                        C[cind + i] = fcalc2(C, beta, i, cind);
                    }
                }
            }
            ...
            static inline void eval(const size_t /* TDM */, const size_t /* TDN */, const size_t _m, const size_t _n, const size_t _k, T1 _alpha, const T2 *_A, const size_t _inca, const T3 *_B, const size_t _ldb, T4 _beta, T5 *_C, const size_t _ldc, OPA &&_opa, OPB &&_opb)
            {
                if (_beta != T4(0.0))
                {
                    if (_alpha != T1(1.0))
                    {
                        ...
                        inner_loop(_m, _n, _k, _alpha, _A, _inca, _B, _ldb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T2 *A, size_t j, const T1 &alpha, OPA &&opa)
                                   { return alpha * opa(A[j]); }, [](const T5 *C, const T6 &opaj, const T3 *B, const T4 &beta, size_t i, size_t cind, size_t bind, OPB &&opb)
                                   { return beta * C[cind + i] + opaj * opb(B[bind + i]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                    else
                    {
                        using T6 = T2;
                        inner_loop(_m, _n, _k, _alpha, _A, _inca, _B, _ldb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T2 *A, size_t j, const T1 & /*alpha*/, OPA &&opa)
                                   { return opa(A[j]); }, [](const T5 *C, const T6 &opaj, const T3 *B, const T4 &beta, size_t i, size_t cind, size_t bind, OPB &&opb)
                                   { return beta * C[cind + i] + opaj * opb(B[bind + i]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                }
                else
                {
                    if (_alpha != T1(1.0))
                    {
                        ...
                        inner_loop(_m, _n, _k, _alpha, _A, _inca, _B, _ldb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T2 *A, size_t j, const T1 &alpha, OPA &&opa)
                                   { return alpha * opa(A[j]); }, [](const T5 * /*C*/, const T6 &opaj, const T3 *B, const T4 & /*beta*/, size_t i, size_t /*cind*/, size_t bind, OPB &&opb)
                                   { return opaj * opb(B[bind + i]); }, [](const T5 * /*C*/, const T4 & /*beta*/, size_t /*i*/, size_t /*cind*/)
                                   { return T5(0.0); });
                    }
                    else
                    {
                    ...
                    if (jb < min_km && jb + TDM < min_km)
                    {
                        for (size_t ib = 0; ib < n; ib += TDN)
                        {
                            for (size_t j = jb; j < jb + TDM; ++j)
                            {
                                ...
                                {
                                    size_t bind = i * ldb;
                                    C[cind + i] = fcalc1(C, opaj, B, beta, j, i, cind, bind, std::forward<OPB>(opb));
                                }
                            }
                        }
                    }
        ...
        public:
            ...
            static inline void eval(const size_t TDM, const size_t TDN, const size_t _m, const size_t _n, const size_t _k, T1 _alpha, const T2 *_A, const size_t _inca, const T3 *_B, const size_t _ldb, T4 _beta, T5 *_C, const size_t _ldc, OPA &&_opa, OPB &&_opb)
            {
                if (_beta != T4(0.0))
                {
                    ...
                    {
                        using T6 = decltype(T1() * _opa(T2()));
                        inner_loop(TDM, TDN, _m, _n, _k, _alpha, _A, _inca, _B, _ldb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T2 *A, size_t j, const T1 &alpha, OPA &&opa)
                                   { return alpha * opa(A[j]); }, [](const T5 *C, const T6 &opaj, const T3 *B, const T4 &beta, size_t j, size_t i, size_t cind, size_t bind, OPB &&opb)
                                   { return beta * C[cind + i] + opaj * opb(B[bind + j]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                    else
                    {
                        using T6 = T2;
                        inner_loop(TDM, TDN, _m, _n, _k, _alpha, _A, _inca, _B, _ldb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T2 *A, size_t j, const T1 & /*alpha*/, OPA &&opa)
                                   { return opa(A[j]); }, [](const T5 *C, const T6 &opaj, const T3 *B, const T4 &beta, size_t j, size_t i, size_t cind, size_t bind, OPB &&opb)
                                   { return beta * C[cind + i] + opaj * opb(B[bind + j]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                }
                else
                {
                    if (_alpha != T1(1.0))
                    {
                        ...
                        inner_loop(TDM, TDN, _m, _n, _k, _alpha, _A, _inca, _B, _ldb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T2 *A, size_t j, const T1 &alpha, OPA &&opa)
                                   { return alpha * opa(A[j]); }, [](const T5 * /*C*/, const T6 &opaj, const T3 *B, const T4 & /*beta*/, size_t j, size_t /*i*/, size_t /*cind*/, size_t bind, OPB &&opb)
                                   { return opaj * opb(B[bind + j]); }, [](const T5 * /*C*/, const T4 & /*beta*/, size_t /*i*/, size_t /*cind*/)
                                   { return T5(0.0); });
                    }
                    else
                    {
            ...
            static inline void eval(const size_t /* TDM */, const size_t /* TDN */, const size_t _m, const size_t _n, const size_t _k, T1 _alpha, const T2 *_A, const size_t _lda, const T3 *_B, const size_t _incb, T4 _beta, T5 *_C, const size_t _ldc, OPA &&_opa, OPB &&_opb)
            {
                if (_beta != T4(0.0))
                {
                    if (_alpha != T1(1.0))
                    {
                        ...
                        inner_loop(_m, _n, _k, _alpha, _A, _lda, _B, _incb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T3 *B, size_t j, const T1 &alpha, OPB &&opb)
                                   { return alpha * opb(B[j]); }, [](const T5 *C, const T6 &opbj, const T2 *A, const T4 &beta, size_t i, size_t cind, size_t aind, OPA &&opa)
                                   { return beta * C[cind + i] + opbj * opa(A[aind + i]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                    else
                    {
                        using T6 = T3;
                        inner_loop(_m, _n, _k, _alpha, _A, _lda, _B, _incb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T3 *B, size_t j, const T1 & /*alpha*/, OPB &&opb)
                                   { return opb(B[j]); }, [](const T5 *C, const T6 &opbj, const T2 *A, const T4 &beta, size_t i, size_t cind, size_t aind, OPA &&opa)
                                   { return beta * C[cind + i] + opbj * opa(A[aind + i]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                }
                else
                {
                    if (_alpha != T1(1.0))
                    {
                        ...
                        inner_loop(_m, _n, _k, _alpha, _A, _lda, _B, _incb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T3 *B, size_t j, const T1 &alpha, OPB &&opb)
                                   { return alpha * opb(B[j]); }, [](const T5 * /*C*/, const T6 &opbj, const T2 *A, const T4 & /*beta*/, size_t i, size_t /*cind*/, size_t aind, OPA &&opa)
                                   { return opbj * opa(A[aind + i]); }, [](const T5 * /*C*/, const T4 & /*beta*/, size_t /*i*/, size_t /*cind*/)
                                   { return T5(0.0); });
                    }
                    else
                    {
                ...
                for (size_t jb = 0; jb < m; jb += TDM)
                {
                    for (size_t ib = 0; ib < n; ib += TDN)
                    {
                        ...
                        else if (ib > min_kn)
                        {
                            ...
                            {
                                size_t cind = ldc * j;
                                for (size_t i = ib; i < ib + TDM && i < n; ++i)
                                {
                                    C[cind + i] = fcalc2(C, beta, i, cind);
                                }
                            }
                        ...
                        else
                        {
                            for (size_t j = jb; j < jb + TDN && j < m; ++j)
                            {
                                ...
                                for (size_t i = min_kn; i < ib + TDM && i < n; ++i)
                                {
                                    C[cind + i] = fcalc2(C, beta, i, cind);
                                }
                            }
        ...
        public:
            ...
            static inline void eval(const size_t TDM, const size_t TDN, const size_t _m, const size_t _n, const size_t _k, T1 _alpha, const T2 *_A, const size_t _lda, const T3 *_B, const size_t _incb, T4 _beta, T5 *_C, const size_t _ldc, OPA &&_opa, OPB &&_opb)
            {
                if (_beta != T4(0.0))
                {
                    ...
                    {
                        using T6 = decltype(T1() * _opb(T3()));
                        inner_loop(TDM, TDN, _m, _n, _k, _alpha, _A, _lda, _B, _incb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T3 *B, size_t j, const T1 &alpha, OPB &&opb)
                                   { return alpha * opb(B[j]); }, [](const T5 *C, const T6 &opbj, const T2 *A, const T4 &beta, size_t j, size_t i, size_t cind, size_t aind, OPA &&opa)
                                   { return beta * C[cind + i] + opbj * opa(A[aind + j]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                    else
                    {
                        using T6 = T3;
                        inner_loop(TDM, TDN, _m, _n, _k, _alpha, _A, _lda, _B, _incb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T3 *B, size_t j, const T1 & /* alpha */, OPB &&opb)
                                   { return opb(B[j]); }, [](const T5 *C, const T6 &opbj, const T2 *A, const T4 &beta, size_t j, size_t i, size_t cind, size_t aind, OPA &&opa)
                                   { return beta * C[cind + i] + opbj * opa(A[aind + j]); }, [](const T5 *C, const T4 &beta, size_t i, size_t cind)
                                   { return C[cind + i] * beta; });
                    }
                }
                else
                {
                    if (_alpha != T1(1.0))
                    {
                        ...
                        inner_loop(TDM, TDN, _m, _n, _k, _alpha, _A, _lda, _B, _incb, _beta, _C, _ldc, std::forward<OPA>(_opa), std::forward<OPB>(_opb), [](const T3 *B, size_t j, const T1 &alpha, OPB &&opb)
                                   { return alpha * opb(B[j]); }, [](const T5 * /*C*/, const T6 &opbj, const T2 *A, const T4 & /*beta*/, size_t j, size_t /* i */, size_t /*cind*/, size_t aind, OPA &&opa)
                                   { return opbj * opa(A[aind + j]); }, [](const T5 * /*C*/, const T4 & /*beta*/, size_t /*i*/, size_t /*cind*/)
                                   { return T5(0.0); });
                    }
                    else
                    {
        ...
        void elementwise_multiplication(const size_t m, T1 alpha, const T2 *A, const size_t inca, const T3 *B, const size_t incb, T4 beta, T5 *C, const size_t incc, OPA &&opa, OPB &&opb)
        {
            if (beta != T4(0.0))
            {
                if (alpha != T1(1.0))
                {
                    ...
                    {
                        size_t ic = i * incc;
                        C[ic] = alpha * opa(A[i * inca]) * opb(B[i * incb]) + beta * C[ic];
                    }
                }
                else
                {
                    ...
                    {
                        size_t ic = i * incc;
                        C[ic] = opa(A[i * inca]) * opb(B[i * incb]) + beta * C[ic];
                    }
                }
            }
            else
            {
                if (alpha != T1(1.0))
                {
                    for (size_t i = 0; i < m; ++i)
                    {
                        C[i * incc] = alpha * opa(A[i * inca]) * opb(B[i * incb]);
                    }
                }
                else
                {
                    for (size_t i = 0; i < m; ++i)
                    {
                        C[i * incc] = opa(A[i * inca]) * opb(B[i * incb]);
                    }
                }
            }
        }
        ...
        {
            template <bool trans_res, size_t MAXDIM>
            struct block_assign;
            template <size_t MAXDIM>
            struct block_assign<false, MAXDIM>
            {
            ...
            template <size_t MAXDIM>
            struct block_assign<true, MAXDIM>
            {
                using size_type = std::size_t;
                using index_type = int;
                template <typename T, typename ELEM_ASSIGN>
        ...
        public:
            ...
            static inline void eval(const size_type _TDM, const size_type _TDN, const size_type m, const size_type n, T alpha, const T *A, const index_type *rowptr, const index_type *colind, const T *B, const size_type ldb, const T beta, T *C, const size_type ldc, OPA &&opa, OPB &&opb)
            {
                ...
                if (alpha == T(1.0))
                {
                    if (beta == T(0.0))
                    {
                        inner_loop(TDM, TDN, Ci, Cj, alpha, A, rowptr, colind, B, ldb, beta, C, ldc, std::forward<OPA>(opa), std::forward<OPB>(opb), bl_ass, [](T &_C, const T & /* _alpha */, const T &_r, const T & /* _beta */)
                                   { _C = _r; });
                    }
                    else
                    {
                    ...
                    for (size_type bj = 0; bj < Cj; bj += TDN)
                    {
                        ...
                        for (size_type ii = 0; ii < TDM && ii + bi < Ci; ++ii)
                        {
                            ...
                            for (size_type jj = 0; jj < TDN && jj + bj < Cj; ++jj)
                            {
                                ...
                                for (index_type index = rowptr[i]; index < rowptr[i + 1]; ++index)
                                {
                                    rbt += opa(A[index]) * opb(B[bskip + colind[index]]);
                                }
                                rbuf[ii * TDN + jj] = rbt;
                            }
                        }
        ...
        public:
            ...
            static inline void eval(const size_type m, T alpha, const T *A, const index_type *rowptr, const index_type *colind, const T *B, const size_type incb, const T beta, T *C, const size_type incc, OPA &&opa, OPB &&opb)
            {
                if (alpha == T(1.0))
                {
                    if (beta == T(0.0))
                    {
                        inner_loop(m, alpha, A, rowptr, colind, B, incb, beta, C, incc, std::forward<OPA>(opa), std::forward<OPB>(opb), [](T &_C, const T & /* _alpha */, const T &_r, const T & /* _beta */)
                                   { _C = _r; });
                    }
                    else
                    {
        ...
        protected:
            ...
            static inline void inner_loop(const size_type m, T alpha, const T *A, const index_type *rowptr, const index_type *colind, const T *B, const size_type incb, const T beta, T *C, const size_type incc, OPA &&opa, OPB &&opb, ELEM_ASSIGN &&elfunc)
            {
                ...
                for (size_type i = 0; i < m; ++i)
                {
                    ...
                    for (index_type index = rowptr[i]; index < rowptr[i + 1]; ++index)
                    {
                        rbt += opa(A[index]) * opb(B[colind[index] * incb]);
                    }
...
#endif // PYTTN_LINALG_BACKENDS_EXTENDED_BLAS_KERNELS_HPP_
