...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_LU_DECOMPOSITION_HPP_
    ...
    namespace internal
    {
        struct lu_result_validation
        {
            ...
            static void validate_ipiv(const matrix &m, ipiv_type &ipiv)
            {
                ...
                if (ipiv.size() != minmn)
                {
                    CALL_AND_HANDLE(ipiv.resize(minmn), "Failed to reshape the pivot array.");
                }
            }
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_LU_DECOMPOSITION_HPP_//
