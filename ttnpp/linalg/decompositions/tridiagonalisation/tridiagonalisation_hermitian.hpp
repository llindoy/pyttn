...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_TRIDIAGONALISATION_TRIDIAGONALISATION_HERMITIAN_HPP_
...
namespace linalg
{
    namespace internal
    {
        ...
        public:
            ...
            void resize_workspace(size_type worksize)
            {
                ...
                {
                    CALL_AND_HANDLE(m_work.resize(worksize), "Failed to resize workspace of generalised_eigensolver object.");
                }
            }
            ...
            valid_decomp_func<matrix_type, mat_type, size_type, validate_vecs_type, Qtype> query_work_space(mat_type &mat, symmetric_tridiagonal_matrix<real_type, backend_type> &tri, Qtype &Q)
            {
                try
                {
                    ...
                    {
                        CALL_AND_RETHROW(return internal::worksize_as_integer(worksize2));
                    }
                    else
                    {
                        CALL_AND_RETHROW(return internal::worksize_as_integer(worksize));
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, tri), "Failed to compute tridiagonal reduction of hermitian matrix.  Failed to determine optimal work space.");
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, tri, Q), "Failed to compute tridiagonal reduction of hermitian matrix.  Failed to determine optimal work space.");
                CALL_AND_HANDLE(resize_workspace(worksize), "Failed to compute tridiagonal reduction of hermitian matrix. Failed to resize the optimal workspace array.");
    ...
    template <typename matrix_type>
    class tridiagonalisation<matrix_type,
                             typename std::enable_if<
                                 ...
                                 void>::type> : public internal::tridiagonalisation_hermitian<remove_cvref_t<matrix_type>>
    {
    public:
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_TRIDIAGONALISATION_TRIDIAGONALISATION_HERMITIAN_HPP_
