...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_SPARSE_ARNOLDI_ITERATION_HPP_
#define PYTTN_LINALG_DECOMPOSITIONS_SPARSE_ARNOLDI_ITERATION_HPP_
...
namespace linalg
{
    ...
    public:
        ...
        void resize(size_type krylov_dim, size_type dim)
        {
            try
            {
                if (krylov_dim > m_max_krylov_dim)
                {
        ...
        bool partial_krylov_step(const vec_type &vec, real_type scale, size_type istart, size_type iend, Args &&...args)
        {
            try
            {
                ...
                for (size_type i = istart; i <= iend && !iteration_completed_early; ++i)
                {
                    ...
                    {
                        auto q0 = m_Q[0];
                        q0 = vec.reinterpret_shape(q0.size());
                        real_type fnorm = sqrt(real(dot_product(conj(q0), q0)));
                        m_beta = fnorm;
                        q0 = q0 / fnorm;
                    }
                    else
                    {
                        ...
                        if (norm < m_threshold)
                        {
                            iteration_completed_early = true;
                        }
                        else
                        {
                            qi = m_w / norm;
        ...
        bool partial_krylov_step(const vecs_type &vec, size_type vindex, real_type scale, size_type istart, size_type iend, bool deflation, Args &&...args)
        {
            try
            {
                ...
                for (size_type i = istart; i <= iend && !iteration_completed_early; ++i)
                {
                    ...
                    {
                        auto q0 = m_Q[0];
                        q0 = vec[vindex].reinterpret_shape(q0.size());
                        real_type fnorm = sqrt(real(dot_product(conj(q0), q0)));
                        m_beta = fnorm;
                        q0 = q0 / fnorm;
    ...
    protected:
        ...
        {
            if (maxsize > m_max_dim)
            {
            } // print an info message telling the user that the krylov subspace will be reshaped resized"
    ...
    public:
        void finalise_krylov_rep(size_type cur_krylov_dim)
        {
            ...
            for (size_type i = 0; i < cur_krylov_dim; ++i)
            {
                ...
                if (i + 1 != cur_krylov_dim)
                {
                    m_H(i, i + 1) = m_Hv(i, i + 1);
                }
            }
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_SPARSE_ARNOLDI_ITERATION_HPP_//
