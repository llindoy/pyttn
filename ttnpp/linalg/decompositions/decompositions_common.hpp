...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_DECOMPOSITIONS_COMMON_HPP_
#define PYTTN_LINALG_DECOMPOSITIONS_DECOMPOSITIONS_COMMON_HPP_
    ...
    namespace internal
    {
        ...
        void unpack_eigenvectors(typename blas_backend::size_type N, T *iw, const T *vecs, complex<T> *out)
        {
            ...
            for (size_type i = 0; i < N; ++i)
            {
                if (iw[i] == T(0.0))
                {
                    set_real_eigenvects(N, vecs + i * N, out + i * N);
                }
                else
                {
                    unpack_complex_eigenvector(N, vecs + i * N, vecs + (i + 1) * N, out + i * N, out + (i + 1) * N);
                    ++i;
                }
            }
        }
        ...
        struct validate_vals_type
        {
            static constexpr bool value = valid_decomposition_vector_type<vals_type, T, B>::value;
        };

        template <typename vals_type, typename T, typename B>
        struct validate_real_vals_type
        {
            static constexpr bool value = valid_decomposition_vector_type<vals_type, typename get_real_type<T>::type, B>::value;
        };
        ...
        struct validate_vecs_type
        {
            static constexpr bool value = valid_decomposition_matrix<vecs_type, T, B>::value;
        };
        ...
        struct validate_vecs_rl_type
        {
            static constexpr bool value = valid_decomposition_matrix<vecs_typer, T, B>::value && valid_decomposition_matrix<vecs_typel, T, B>::value;
        };
        ...
        struct validate_vals_vecs_type
        {
            static constexpr bool value = valid_decomposition_vector_type<vals_type, T, B>::value && valid_decomposition_matrix<vecs_type, T, B>::value;
        };

        template <typename vals_type, typename vecs_typer, typename vecs_typel, typename T, typename B>
        struct validate_vals_vecs_rl_type
        {
            static constexpr bool value = valid_decomposition_vector_type<vals_type, T, B>::value && valid_decomposition_matrix<vecs_typer, T, B>::value && valid_decomposition_matrix<vecs_typel, T, B>::value;
        };
        ...
        template <typename vals_type, typename vecs_type, typename T, typename B>
        struct validate_real_vals_vecs_type
        {
            static constexpr bool value = valid_decomposition_vector_type<vals_type, typename get_real_type<T>::type, B>::value && valid_decomposition_matrix<vecs_type, T, B>::value;
        };

        template <typename vals_type, typename vecs_typer, typename vecs_typel, typename T, typename B>
        struct validate_real_vals_vecs_rl_type
        {
            static constexpr bool value = valid_decomposition_vector_type<vals_type, typename get_real_type<T>::type, B>::value && valid_decomposition_matrix<vecs_typer, T, B>::value && valid_decomposition_matrix<vecs_typel, T, B>::value;
        };

        template <typename m1, typename m2>
        struct is_valid_decomp_matrix_type
        {
            static constexpr bool value = (same_topology_type<m1, m2>::value && compatible_traits<m1, m2>::value);
        };
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_DECOMPOSITIONS_COMMON_HPP_
