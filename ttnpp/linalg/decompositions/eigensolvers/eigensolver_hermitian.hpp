...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_EIGENSOLVERS_EIGENSOLVER_HERMITIAN_HPP_
...
namespace linalg
{
    namespace internal
    {
        ...
        public:
            ...
            void resize(size_type n, bool requires_matrix = true)
            {
                try
                {
                    if (requires_matrix)
                    {
                        CALL_AND_HANDLE(m_mat.resize(n, n), "Failed whenn resizing internal matrix.");
                    }
        ...
        protected:
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, eigs), "Failed to compute eigenvalues of hermitian matrix.  Failed to determine optimal work space.");
                if (m_work.size() < worksize)
                {
                    CALL_AND_HANDLE(m_work.resize(worksize), "Failed to compute eigenvalues of hermitian matrix. Failed to resize the optimal workspace array.");
                }
                char JOBZ = 'N';
                char UPLO = 'U';
                int_type n = mat.shape(0);
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, eigs, vecs), "Failed to compute eigendecomposition of hermitian matrix.  Failed to determine optimal work space.");
                if (m_work.size() < worksize)
                {
                    CALL_AND_HANDLE(m_work.resize(worksize), "Failed to compute eigendecomposition of hermitian matrix. Failed to resize the optimal workspace array.");
                }
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_EIGENSOLVERS_EIGENSOLVER_HERMITIAN_HPP_//
