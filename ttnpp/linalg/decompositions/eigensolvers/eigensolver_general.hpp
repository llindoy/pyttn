...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_EIGENSOLVERS_EIGENSOLVER_GENERAL_HPP_
...
namespace linalg
{
    namespace internal
    {
        ...
        protected:
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat), "Failed to compute eigenvalues of general matrix.  Failed to determine optimal work space.");
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, vecs), "Failed to compute eigenvalues of general matrix.  Failed to determine optimal work space.");
                CALL_AND_HANDLE(resize_workspace(worksize), "Failed to compute eigendecomposition of general matrix. Failed to resize the optimal workspace array.");
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, vecs_l, vecs_r), "Failed to compute eigendecomposition of general matrix.  Failed to determine optimal work space.");
                CALL_AND_HANDLE(resize_workspace(worksize), "Failed to compute eigendecomposition of general matrix. Failed to resize the optimal workspace array.");
        ...
        public:
            ...
            void resize(size_type n, bool requires_matrix = true, bool requires_scaling = false)
            {
                try
                {
                    ...
                    if (requires_scaling)
                    {
                        CALL_AND_HANDLE(m_scaling.resize(n, n), "Failed when resizing the internal scaling matrix required to give sensibly normalised eigenvectors.");
                    }
        ...
        protected:
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, eigs), "Failed to compute eigenvalues of general matrix.  Failed to determine optimal work space.");
                if (m_work.size() < worksize)
                {
                    CALL_AND_HANDLE(m_work.resize(worksize), "Failed to compute eigenvalues of general matrix. Failed to resize the optimal workspace array.");
                }
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, eigs, vecs), "Failed to compute eigendecomposition of general matrix.  Failed to determine optimal work space.");
                if (m_work.size() < worksize)
                {
                    CALL_AND_HANDLE(m_work.resize(worksize), "Failed to compute eigendecomposition of general matrix. Failed to resize the optimal workspace array.");
                }
            ...
            {
                size_type worksize;
                CALL_AND_HANDLE(worksize = query_work_space(mat, eigs, vecs_r, vecs_l), "Failed to compute eigendecomposition of general matrix.  Failed to determine optimal work space.");
                if (m_work.size() < worksize)
                {
                    CALL_AND_HANDLE(m_work.resize(worksize), "Failed to compute eigendecomposition of general matrix. Failed to resize the optimal workspace array.");
                }
    ...
    class eigensolver<matrix_type,
                      typename std::enable_if<
                          ...
                          void>::type> : public internal::general_eigensolver<remove_cvref_t<matrix_type>>
    {
    public:
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_EIGENSOLVERS_EIGENSOLVER_GENERAL_HPP_//
