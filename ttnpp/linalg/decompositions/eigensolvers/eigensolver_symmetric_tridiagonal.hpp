...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_EIGENSOLVERS_EIGENSOLVER_SYMMETRIC_TRIDIAGONAL_HPP_
            ...
            void resize(size_type n, bool compute_eigenvectors = true, bool keep_input = true)
            {
                try
                {
                    if (keep_input)
                    {
                        if (n > m_E.size() + 1)
                        {
                            CALL_AND_HANDLE(m_E.resize(n - 1), "Failed when resizing internal E buffer.");
                        }
                    }
                    if (compute_eigenvectors)
                    {
                        size_type worksize = 2 * n - 2;
                        if (worksize > m_work.size())
                        {
                            CALL_AND_HANDLE(m_work.resize(1 > worksize ? 1 : worksize), "Failed when resizing the internal working buffer.");
                        }
                    }
                }
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_EIGENSOLVERS_EIGENSOLVER_SYMMETRIC_TRIDIAGONAL_HPP_//
