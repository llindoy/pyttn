...
#ifndef PYTTN_LINALG_DECOMPOSITIONS_GENERALISED_EIGENSOLVERS_GENERALISED_EIGENSOLVER_BASE_HPP_
#define PYTTN_LINALG_DECOMPOSITIONS_GENERALISED_EIGENSOLVERS_GENERALISED_EIGENSOLVER_BASE_HPP_
...
namespace linalg
{
    namespace internal
    {
        ...
        struct generalised_eigensolver_result_validation
        {
            template <typename matrix_type, typename vals_type>
            static inline
                typename std::enable_if<is_diagonal_matrix_type<vals_type>::value &&
                                            traits<vals_type>::is_resizable,
                                        void>::type
                eigenvalues(const matrix_type &mat, vals_type &eigs)
            {
                if ((eigs.shape(0) != eigs.shape(1)) || (mat.shape(0) != eigs.nelems()))
                {
                    ...
                    CALL_AND_HANDLE(eigs.resize(mat.shape(0)),
            ...
            template <typename matrix_type, typename vals_type>
            static inline typename std::enable_if<is_dense_tensor<vals_type>::value &&
                                                      traits<vals_type>::rank == 1 &&
                                                      traits<vals_type>::is_resizable,
                                                  void>::type
            eigenvalues(const matrix_type &mat, vals_type &eigs)
            {
            ...
            eigenvalues(const matrix_type &mat, vals_type &eigs)
            {
                ASSERT(mat.shape(0) == eigs.shape(0),
                       "The input eigenvalues array does not have the correct shape.");
            }
            ...
            static inline
                ...
                eigenvectors(const matrix_type &mat, vecs_type &vecs)
            {
...
#endif // PYTTN_LINALG_DECOMPOSITIONS_GENERALISED_EIGENSOLVERS_GENERALISED_EIGENSOLVER_BASE_HPP_//
