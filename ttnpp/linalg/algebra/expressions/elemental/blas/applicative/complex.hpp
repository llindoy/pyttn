...
namespace linalg
{
    ...
    namespace expression_templates
    {
        ...
        public:
            ...
            static inline auto apply(const T1 *a, const T2 *b, size_type i) -> std::complex<decltype(a[i] + b[i])> { return std::complex<decltype(a[i] + b[i])>(a[i], b[i]); }

            template <typename T, typename ltype, typename rtype, template <typename> class op>
            static inline auto apply(const T *a, binary_expression<ltype, rtype, op, blas_backend> b, size_type i) -> std::complex<decltype(a[i] + b[i])> { return std::complex<decltype(a[i] + b[i])>(a[i], b[i]); }

            template <typename T, typename vtype, template <typename> class op>
            static inline auto apply(const T *a, unary_expression<vtype, op, blas_backend> b, size_type i) -> std::complex<decltype(a[i] + b[i])> { return std::complex<decltype(a[i] + b[i])>(a[i], b[i]); }
        };
        ...
        public:
            template <typename T1, typename T2>
            static inline auto apply(const T1 *a, const T2 *b, size_type i) -> std::complex<decltype(a[i] + b[i])> { return polar(a[i], b[i]); }

            template <typename T, typename ltype, typename rtype, template <typename> class op>
            static inline auto apply(const T *a, binary_expression<ltype, rtype, op, blas_backend> b, size_type i) -> std::complex<decltype(a[i] + b[i])> { return polar(a[i], b[i]); }

            template <typename T, typename vtype, template <typename> class op>
            static inline auto apply(const T *a, unary_expression<vtype, op, blas_backend> b, size_type i) -> std::complex<decltype(a[i] + b[i])> { return polar(a[i], b[i]); }
        };
        ...
        public:
            template <typename T>
            static inline auto apply(const T *a, size_type i) -> std::complex<decltype(a[i])> { return std::complex<decltype(a[i])>(cos(a[i]), sin(a[i])); }
            template <typename T>
            static inline auto apply(const T &a, size_type i) -> std::complex<decltype(a[i])> { return std::complex<decltype(a[i])>(cos(a[i]), sin(a[i])); }
        };
