...
namespace linalg
{
    namespace expression_templates
    {
        ...
        template <>
        struct requires_working_buffer<cuda_backend>
        {
        ...
        protected:
            ...
            device_value_type* m_working_buffer;
            device_value_type* m_conj_buffer;
            ...
            void initialise_state(const left_type &A, const right_type &B, size_type ind, bool conjA, bool conjB)
            {
                try
                {
                    ...
                    for (size_type i = 0; i < 3; ++i)
                    {
                        if (i != ind)
                        {
                            ASSERT(A.shape(i) == B.shape(i), "Failed to construct rank 3, rank 3 tensor contraction expression.  The contraction indices do not have the same dimensions.");
                        }
                    }
                    this->m_shape[0] = A.shape(ind);
                    this->m_shape[1] = B.shape(ind);
                    ...
                    case (0):
                    {
                        ...
                        m_outer = false;
                        m_Abuffer = A.buffer();
                        m_ldA = A.shape(1) * A.shape(2);
                        m_Atotsize = A.size();
                        m_Bbuffer = B.buffer();
                        m_ldB = B.shape(1) * B.shape(2);
                        m_Btotsize = B.size();
        ...
        public:
                    ...
                    else
                    {
                        ...
                        if (m_opA == backend_type::op_c && m_opB != backend_type::op_c)
                        {
                            ...
                            if (is_complex<value_type>::value)
                            {
                                ...
                                backend_algebra<backend_type>::complex_conjugate(m_Atotsize, m_Abuffer, m_conj_buffer);
                        ...
                        else if (m_opA != backend_type::op_c && m_opB == backend_type::op_c)
                        {
                            ...
                            if (is_complex<value_type>::value)
                            {
                                ...
                                backend_algebra<backend_type>::complex_conjugate(m_Btotsize, m_Bbuffer, m_conj_buffer);
            ...
            {
                return m_outer && requires_working_buffer<backend_type>::value();
            }
            size_type working_size() const {return m_t*m_m*m_n;}
        protected:
            template <typename T3>
            void eval_contraction(T3 &res, value_type _beta, value_type _coeff, value_ptr Abuffer, value_ptr Bbuffer, ttype opA, ttype opB)
            {
                ...
                try
                {
                    if (m_outer)
                    {
                        ...
                        CALL_AND_HANDLE(
                            backend_algebra<backend_type>::outer_contract(opB, opA, m_m, m_n, m_k, coeff, Bbuffer, m_ldB, m_strideB, Abuffer, m_ldA, m_strideA, beta, m_working_buffer, ldres, strideres, m_t, res.buffer()),
...
#endif // PYTTN_LINALG_ALGEBRA_EXPRESSIONS_CONTRACTIONS_CONTRACTION_332_HPP_//
