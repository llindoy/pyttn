...
#ifndef PYTTN_LINALG_ALGEBRA_EXPRESSIONS_CONTRACTIONS_DIAGONAL_MATRIX_DENSE_MATRIX_MULTIPLICATION_HPP_
#define PYTTN_LINALG_ALGEBRA_EXPRESSIONS_CONTRACTIONS_DIAGONAL_MATRIX_DENSE_MATRIX_MULTIPLICATION_HPP_
...
namespace linalg
{
    ...
    namespace expression_templates
    {
        ...
        class matrix_matrix_product<sparse_type, dense_type,
                                    typename std::enable_if<is_dense_matrix<dense_type>::value && is_diagonal_matrix_type<sparse_type>::value && is_same_backend<sparse_type, dense_type>::value && is_sparse_multiply_value<sparse_type, dense_type>::value, void>::type> : public matrix_matrix_product_base<matrix_matrix_product<sparse_type, dense_type, void>>
        {
        public:
        ...
        public:
            ...
            void applicative_impl(T3 &res, value_type _beta = 0.0, value_type coeff_scale = 1.0)
            {
                try
                {
                    ...
                    CALL_AND_HANDLE(backend_algebra<backend_type>::dgmm(m_sparse_left, m_opA, m_opB, m_n, m_m, m_k, coeff, m_Abuffer, m_incA, m_Bbuffer, m_ldB, beta, res.buffer(), ldc), "dgmm call failed.");
                }
                catch (const std::exception &ex)
                {
                    logging::error(ex.what());
...
#endif // PYTTN_LINALG_ALGEBRA_EXPRESSIONS_CONTRACTIONS_DIAGONAL_MATRIX_DENSE_MATRIX_MULTIPLICATION_HPP_
