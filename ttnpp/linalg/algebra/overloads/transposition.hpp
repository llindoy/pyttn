...
#ifndef PYTTN_LINALG_ALGEBRA_OVERLOADS_TRANSPOSITION_HPP_
#define PYTTN_LINALG_ALGEBRA_OVERLOADS_TRANSPOSITION_HPP_
...
namespace linalg
{
    ...
    trans_return_type<array_type, false> trans(const array_type &a)
    {
        using rettype = trans_type<array_type, false>;
        CALL_AND_RETHROW(return rettype(a));
    }

    // transpose of conjugate is conjugate transpose
    template <typename array_type>
    trans_return_type<array_type, true> trans(const conj_type<array_type> &a)
    {
        using rettype = trans_type<array_type, true>;
        CALL_AND_RETHROW(return rettype(a.obj()));
    }
    ...
    template <typename array_type>
    trans_return_type<array_type, true> adjoint(const array_type &a)
    {
        using rettype = trans_type<array_type, true>;
        CALL_AND_RETHROW(return rettype(a));
    }
    ...
    template <typename array_type>
    typename std::enable_if<array_type::rank == 2, conj_type<array_type>>::type adjoint(const trans_type<array_type, false> &a) { CALL_AND_RETHROW(return conj(a.matrix())); }
    ...
    gemm_return_type<T1, T2> trans(const expression_templates::matrix_matrix_product<T1, T2> &r)
    {
        using rettype = gemm_type<T1, T2>;
        CALL_AND_RETHROW(return rettype(r, true));
    }

    template <typename T1, typename T2>
    gemm_return_type<T1, T2> adjoint(const expression_templates::matrix_matrix_product<T1, T2> &r)
    {
        using rettype = gemm_type<T1, T2>;
        CALL_AND_RETHROW(return rettype(r, true, true));
    }

    // transpose of scalar*conjugate
    template <typename T1, typename T2>
    trans_return_type<T2, true> trans(const scalconj_type<T1, T2> &a)
    {
        using rettype = trans_type<T2, true>;
        CALL_AND_RETHROW(return rettype(a.right().obj(), static_cast<typename T2::value_type>(a.left())));
    }
...
#endif // PYTTN_LINALG_ALGEBRA_OVERLOADS_TRANSPOSITION_HPP_//
