...
#ifndef PYTTN_LINALG_ALGEBRA_OVERLOADS_TENSOR_CONTRACTIONS_HPP_
#define PYTTN_LINALG_ALGEBRA_OVERLOADS_TENSOR_CONTRACTIONS_HPP_

#include "expression_type_aliases.hpp"
...
namespace linalg
{
    ...
    {
        static_assert(std::is_integral<I1>::value && std::is_integral<I2>::value, "Failed to instantiate contraction indices are not integral.");
        using rettype = scal_mtc1_return_type<T1, T2, T3>;
        using value_type = typename rettype::value_type;
    ...
    {
        static_assert(std::is_integral<I1>::value && std::is_integral<I2>::value, "Failed to instantiate contraction indices are not integral.");
        using rettype = scal_scal_mtc1_return_type<T1, T2, T3, T4>;
        using value_type = typename rettype::value_type;
    ...
    ttc2_return_type<T1, T2> contract(const T1 &l, I1 il, I2 jl, const T2 &r, I3 ir, I4 jr)
    {
        ...
        using rettype = ttc2_return_type<T1, T2>;
        using value_type = typename rettype::value_type;
        using size_type = typename rettype::size_type;
