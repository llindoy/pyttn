...
namespace linalg
{
    ...
    public:
        ...
        using index_pointer = typename std::add_pointer<index_type>::type;
        using const_index_pointer = typename std::add_pointer<typename std::add_const<index_type>::type>::type;
        ...
        template <typename U>
        friend class csr_topology_type;

    protected:
        using iallocator = memory::allocator<index_type, backend_type>;
    ...
    public:
        ...
        {
            src.m_colind = nullptr;
            src.m_rowptr = nullptr;
            src.m_nnz = 0;
            src.m_max_nnz = 0;
            src.m_max_rows = 0;
        ...
        ~csr_topology_type()
        {
            try
            {
                if (m_colind != nullptr)
                {
                    iallocator::deallocate(m_colind);
                }
            }
            catch (...)
            {
            ...
            try
            {
                if (m_rowptr != nullptr)
                {
                    iallocator::deallocate(m_rowptr);
                }
            }
            catch (...)
            {
        ...
        self_type &operator=(const self_type &src)
        {
            if (&src != this)
            {
                CALL_AND_HANDLE(copy_assign_impl(src), "Failed to copy assign csr topology base type.");
            }
            return *this;
        }
        template <typename bck, typename = typename std::enable_if<!std::is_same<bck, backend_type>::value, void>::type>
        inline self_type &operator=(const csr_topology_type<bck> &src)
        {
            CALL_AND_HANDLE(copy_assign_impl(src), "Failed to copy assign csr topology base type.");
            return *this;
        }
        ...
        inline self_type &swap(self_type &src) noexcept
        {
            ...
            if (this != &src)
            {
                swap(m_nnz, src.m_nnz);
                swap(m_max_nnz, src.m_max_nnz);
                swap(m_max_rows, src.m_max_rows);
                swap(m_shape, src.m_shape);
                index_pointer temp = src.m_colind;
                src.m_colind = m_colind;
                m_colind = temp;
                temp = src.m_rowptr;
                src.m_rowptr = m_rowptr;
                m_rowptr = temp;
            }
            return *this;
        }
        ...
        inline void deallocate()
        {
            if (m_rowptr != nullptr)
            {
                CALL_AND_HANDLE(iallocator::deallocate(m_rowptr), "Failed to deallocate csr topology object.  Error when deallocating rowptr buffer.");
            }
            m_rowptr = nullptr;
            if (m_colind != nullptr)
            {
                CALL_AND_HANDLE(iallocator::deallocate(m_colind), "Failed to deallocate csr topology object.  Error when deallocating colind buffer.");
            }
            m_colind = nullptr;
            m_nnz = 0;
            m_max_nnz = 0;
        ...
        inline void resize(size_type nrows, size_type ncols)
        {
            if (nrows > m_max_rows)
            {
                if (m_rowptr != nullptr)
                {
                    CALL_AND_HANDLE(iallocator::deallocate(m_rowptr), "Failed to resize the csr topology object.  Failed to deallocate the previous rowptr buffer object.");
                }
        ...
        inline bool resize(size_type nnz)
        {
            ...
            if (nnz > m_max_nnz)
            {
                if (m_colind != nullptr)
                {
                    CALL_AND_HANDLE(iallocator::deallocate(m_colind), "Failed to resize the csr topology object.  Failed to deallocate the previous colind buffer object.");
                }
        ...
        inline void resize(size_type nnz, size_type nrows, size_type ncols)
        {
            CALL_AND_RETHROW(resize(nnz));
            CALL_AND_RETHROW(resize(nrows, ncols));
        }
        ...
        inline void shrink_to_fit()
        {
            ...
            {
                index_pointer temp;
                CALL_AND_HANDLE(temp = iallocator::allocate(m_nnz), "Failed to resize the csr topology object.  Failed to allocate the new colind buffer when an increase in size is required.");
            ...
            if (m_shape[0] < m_max_rows)
            {
                ...
                index_pointer temp;
                CALL_AND_HANDLE(temp = iallocator::allocate(m_shape[0] + 1), "Failed to resize the csr topology object.  Failed to allocate the new rowptr buffer when an increase in size is required.");
        ...
        void set_colind(const std::vector<index_type> &colind)
        {
            ...
            for (size_type i = 0; i < m_shape[0](); ++i)
            {
                ...
                for (index_type j = m_rowptr[i] + 1; j < m_rowptr[i + 1]; ++j)
                {
                    ASSERT(colind[j] > pval, "Failed to set csr topology colind from vector.  The colind array is not correctly formatted.");
                }
            }
        ...
        void set_rowptr(const std::vector<index_type> &rowptr)
        {
            ...
            for (index_type i = 0; i < rowptr.size() - 1; ++i)
            {
                ASSERT(rowptr[i] <= rowptr[i + 1], "Failed to set csr topology rowptr from vector.  An element of the rowptr array is smaller than its proceeding value.");
            }
    ...
    public:
        ...
        csr_matrix_base(size_type _nnz, size_type _nrows, size_type _ncols = 0)
        try : m_vals(nullptr), m_topo(_nnz, _nrows, _ncols)
        {
        ...
        self_type &operator=(const self_type &src)
        {
            if (&src != this)
            {
                CALL_AND_HANDLE(copy_assign_impl(src), "Failed to copy assign csr matrix base type.");
            }
            return *this;
        }
        template <typename Container>
        inline other_copy_assignable_type<Container, self_type> operator=(const Container &src)
        {
            CALL_AND_HANDLE(copy_assign_impl(src), "Failed to copy assign csr matrix base type.");
            return *this;
        }
        ...
        inline void deallocate()
        {
            if (m_vals != nullptr)
            {
                CALL_AND_HANDLE(allocator::deallocate(m_vals), "Failed to deallocate csr matrix object.  Error when deallocating vals buffer.");
            }
            m_vals = nullptr;
        ...
        {
            bool resized;
            CALL_AND_HANDLE(resized = m_topo.resize(_nnz), "Failed to resize the csr matrix object.  Failed when resizing the topology array.");
            if (resized)
            {
                CALL_AND_RETHROW(resize_buffer(_nnz));
            }
        }
        template <typename backend>
        void resize(const csr_topology_type<backend> &_topology)
        {
            if (_topology.m_nnz > m_topo.m_max_nnz)
            {
                CALL_AND_HANDLE(resize_buffer(_topology.m_nnz), "Failed to resize csr matrix object from topology object.  Failed to resize the values buffer.");
            }
        ...
        void resize(size_type _nnz, size_type _nrows, size_type _ncols)
        {
            CALL_AND_RETHROW(resize(_nnz));
            CALL_AND_RETHROW(resize(_nrows, _ncols));
        }
        ...
        {
            ASSERT(i < rank, "Unable to access csr_matrix size element.  Index out of bounds.");
            return m_topo.m_shape[i];
        }
        ...
        void init(const std::vector<value_type> &_vals, const std::vector<index_type> &_colinds, const std::vector<index_type> &_rowptr, size_type _ncols = 0)
        {
            ...
            ASSERT(_vals.size() == static_cast<size_type>(_rowptr[_rowptr.size() - 1]), "Failed to initialise csr_matrix_base object.  The final element of the rowptr array is not equal to the number of non-zeros.");
            ...
            for (size_type i = 0; i < _rowptr.size() - 1; ++i)
            {
                ...
                size_type ninrow = _rowptr[i + 1] - _rowptr[i];
                if (ninrow > max_per_row)
                {
                    max_per_row = ninrow;
                }
            }
            ...
            for (size_type i = 0; i < _rowptr.size() - 1; ++i)
            {
                ...
                {
                    row_indexer[j] = j;
                    row_colinds[j] = _colinds[j + _rowptr[i]];
                }
                ...
                index_type prev_val = row_colinds[0];
                row_colinds[0] = _colinds[_rowptr[i]];
                ...
                for (size_type j = 0; j < ninrow; ++j)
                {
                    if (_ncols != 0)
                    {
                        ASSERT(static_cast<size_type>(row_colinds[j]) < _ncols, "Failed to initialise csr_matrix_base object.  The colinds array contains an element that is out of bounds.");
                    }
                    else
                    {
                        m_topo.m_shape[1] = static_cast<size_type>(row_colinds[j]) + 1 > m_topo.m_shape[1] ? row_colinds[j] + 1 : m_topo.m_shape[1];
                    }
                }
            }
            ...
            m_topo.m_max_nnz = _vals.size();
            m_topo.m_nnz = m_topo.m_max_nnz;
            m_topo.m_max_rows = _rowptr.size() - 1;
            m_topo.m_shape[0] = m_topo.m_max_rows;
            ...
            for (size_type i = 0; i < _rowptr.size() - 1; ++i)
            {
                ...
                CALL_AND_HANDLE(std::sort(row_indexer.begin(), row_indexer.begin() + ninrow, [&row_colinds](size_type lhs, size_type rhs)
                                          { return row_colinds[lhs] < row_colinds[rhs]; }),
                                "Failed to initialise csr_matrix_base object.   Failed to sort the column indices.");
        ...
        void init(const std::vector<std::tuple<index_type, index_type, value_type>> &_coo, size_type _nrows, size_type _ncols = 0)
        {
            ...
            CALL_AND_HANDLE(std::sort(coo.begin(), coo.end(), [](const std::tuple<index_type, index_type, value_type> &lhs, const std::tuple<index_type, index_type, value_type> &rhs)
                                      {if(std::get<0>(lhs) != std::get<0>(rhs)){return (std::get<0>(lhs) < std::get<0>(rhs));}else{return (std::get<1>(lhs) < std::get<1>(rhs));} }),
                            "Failed to initialise csr_matrix_base object.  Failed to sort the coordinate form array array.");
            ...
            for (size_type i = 0; i < coo.size(); ++i)
            {
                ...
                if (_ncols != 0)
                {
                    ASSERT(coordsi[1] < static_cast<index_type>(_ncols), "Failed to initialise csr_matrix_base object.  There is a column index which is out of bounds.");
                }
                else
                {
                    _shape[1] = static_cast<size_type>(coordsi[1] + 1 > static_cast<index_type>(_shape[1]) ? coordsi[1] + 1 : _shape[1]);
                }
                if (_nrows != 0)
                {
                    ASSERT(coordsi[0] < static_cast<index_type>(_nrows), "Failed to initialise csr_matrix_base object.  There is a row index which is out of bounds.");
                }
                else
                {
                    _shape[0] = static_cast<size_type>(coordsi[0] + 1 > static_cast<index_type>(_shape[0]) ? coordsi[0] + 1 : _shape[0]);
                }
            }
    ...
    private:
        ...
        inline typename std::enable_if<is_tensor<Container>::value && is_same_value<Container, self_type>::value, copy_assignable_type<Container, self_type>>::type copy_assign_impl(const Container &src)
        {
            ...
            bool assign_reallocated;
            CALL_AND_HANDLE(assign_reallocated = m_topo.copy_assign_impl(src.m_topo), "Failed to copy assign csr_matrix_base object.  Failed when copy assigning the topology array.");
            if (assign_reallocated)
            {
        ...
        inline typename std::enable_if<is_expression<Container>::value, copy_assignable_type<Container, self_type>>::type copy_assign_impl(const Container &expr)
        {
            ...
            CALL_AND_HANDLE(requires_resize = !(expr.shape() == m_topo), "Copy assignment failed. Failed to determine whether the csr matrix requires resizing before evaluation of the input expression.");
            if (requires_resize)
            {
                CALL_AND_HANDLE(resize(expr.shape()), "Copy assignment failed.  Failed to resize the csr matrix object.");
            }
        ...
        void resize_buffer(size_type _nnz)
        {
            if (m_vals != nullptr)
            {
                CALL_AND_HANDLE(allocator::deallocate(m_vals), "Failed to resize the csr matrix object.  Failed to deallocate the previous vals buffer object.");
            }
