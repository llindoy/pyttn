...
namespace linalg
{
    ...
    private:
        ...
        inline typename std::enable_if<is_expression<Container>::value, copy_assignable_type<Container, self_type>>::type copy_assign_impl(const Container &expr)
        {
            static_assert(traits<self_type>::is_mutable, "Failed to initialise copy assignment operator for tensor_view object.  The specified tensor view is not mutable.");
            shape_type _shape;
            CALL_AND_HANDLE(_shape = expr.shape(), "Failed to copy assign tensor_view object.  Failed to get the shape of the expression object.");
            for (size_type i = 0; i < rank; ++i)
            {
                ASSERT(shape(i) == _shape[i], "Failed to copy assign tensor_view object.  The two slice object do not have the same shape.");
            }
    ...
    private:
        ...
        addition_assign_impl(const Container &expr)
        {
            static_assert(traits<self_type>::is_mutable, "Failed to initialise addition assignment operator for tensor_view object.  The specified tensor slice is not mutable.");
            shape_type _shape;
            CALL_AND_HANDLE(_shape = expr.shape(), "Failed to addition assign tensor_view object.  Failed to get the shape of the expression object.");
            for (size_type i = 0; i < rank; ++i)
            {
                ASSERT(shape(i) == _shape[i], "Failed to addition assign tensor_view object.  The two slice object do not have the same shape.");
            }
    ...
    private:
        ...
        subtraction_assign_impl(const Container &expr)
        {
            static_assert(traits<self_type>::is_mutable, "Failed to initialise subtraction assignment operator for tensor_view object.  The specified tensor slice is not mutable.");
            shape_type _shape;
            CALL_AND_HANDLE(_shape = expr.shape(), "Failed to subtraction assign tensor_view object.  Failed to get the shape of the expression object.");
            for (size_type i = 0; i < rank; ++i)
            {
                ASSERT(shape(i) == _shape[i], "Failed to subtraction assign tensor_view object.  The two slice object do not have the same shape.");
            }
        ...
        inline typename std::enable_if<is_expression<Container>::value, move_assignable_type<Container, self_type>>::type subtraction_assign_impl(Container &&expr)
        {
            ...
            shape_type _shape;
            CALL_AND_HANDLE(_shape = expr.shape(), "Failed to subtract assign tensor_view object.  Failed to get the shape of the expression object.");
            for (size_type i = 0; i < rank; ++i)
            {
                ASSERT(shape(i) == _shape[i], "Failed to subtract assign tensor_view object.  The two slice object do not have the same shape.");
            }
    ...
    private:
        ...
        inline void init_stride()
        {
            ...
            {
                m_stride[D - i - 1] = m_shape[D - i] * m_stride[D - i];
            }
        }

        // Function for initialising the array
        template <size_type d, typename U, typename... Args>
        typename std::enable_if<std::is_integral<U>::value, void>::type init_shape(U i, Args &&...args)
        {
            m_shape[d] = i;
            CALL_AND_HANDLE(init_shape<d + 1>(std::forward<Args>(args)...), "Failed to evaluate intermediate init call.");
        }
    ...
    class reinterpreted_tensor : public tensor_view<T, D, backend>
    {
    public:
        using value_type = T;
        ...
        template <typename... Args>
        reinterpreted_tensor(Args &&...args)
        try : base_type(std::forward<Args>(args)...) {}
        catch (const std::exception &ex)
        {
            std::cerr << ex.what() << std::endl;
    ...
    class hermitian_matrix : public tensor_view<T, 2, backend>, public hermitian_type
    {
    public:
        using value_type = T;
        using backend_type = backend;
    ...
    // hessenberg matrix).
    template <typename T, typename backend>
    class upper_hessenberg_matrix : public tensor_view<T, 2, backend>, public upper_hessenberg_type
    {
    public:
        using value_type = T;
        using backend_type = backend;
        ...
        template <typename... Args>
        upper_hessenberg_matrix(Args &&...args)
        ...
        template <typename... Args>
        upper_hessenberg_matrix(MATRIX_ORDERING morder, Args &&...args)
