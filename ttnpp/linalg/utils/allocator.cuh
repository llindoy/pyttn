...
namespace linalg
{
    namespace memory
    {
        ...
        public:
            CudaDeviceAllocator(int mpi_rank = 0, int gpu_id = 0) : m_domain{mpi_rank, gpu_id}
            {
                ASSERT(gpu_id >= 0, "Cannot setup CudaDeviceAllocator to point to a cpu context.");
            }

            ExecDomain domain() const override{return m_domain;}

            void* allocate_bytes(std::size_t n, std::size_t /*alignment*/) override
            {
                CALL_AND_HANDLE(cuda_safe_call(cudaSetDevice(m_domain.gpu_id())), "Failed to set cuda device id.");
        ...
        public:
            PinnedHostAllocator(int mpi_rank = 0) : m_domain{mpi_rank, -1}{}
        ...
        public:
            CudaCircularBufferAllocator(int mpi_rank = 0, int gpu_id = 0, std::size_t capacity_bytes) : m_domain{mpi_rank, gpu_id}, m_capacity(capacity_bytes), m_offset(0), m_buffer(nullptr)
            {
                ASSERT(gpu_id >= 0, "Cannot setup CudaCircularBufferAllocator to point to a cpu context.");
                CALL_AND_HANDLE(cuda_safe_call(cudaSetDevice(gpu_id)), "Failed to set cuda device id.");
